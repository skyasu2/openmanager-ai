...
포트폴리오 관점에서 이 프로젝트는 단순 UI 데모가 아니라, **Next.js + Cloud Run + multi-agent AI + 에이전트 협업 개발**을 실제 배포·CI/CD 흐름으로 묶은 결과물입니다.
...
5개 라우팅 에이전트가 협업하여 복잡한 질문도 처리합니다. **Vercel AI SDK v6**의 네이티브 멀티-에이전트 패턴으로 구현되었습니다.
