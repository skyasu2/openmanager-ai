...
`qa:status`는 read-only 요약 조회이고, `npm run qa:status -- --write --sync-public` 또는 `npm run qa:evidence:build`는 public snapshot을 명시적으로 수동 재생성합니다.
