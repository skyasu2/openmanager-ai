...
## Regression Checklist Additions (2026-03-17)

- Landing / Profile bootstrap state coherence
  - 목적: 랜딩 첫 진입 시 인증 상태가 확정되기 전 `? / 로딩 중 / 확인 중...` 같은 임시 텍스트가 노출되지 않는지 확인
  - 기대값: guest/user 최종 상태 또는 중립 skeleton만 보이고, 의미 없는 placeholder 텍스트는 노출되지 않음
  - 근거: `QA-20260317-0119` Playwright MCP 실측에서 첫 `0~200ms` 구간에 임시 텍스트가 관찰됨
