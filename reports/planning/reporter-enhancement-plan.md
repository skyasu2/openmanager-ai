...
- Reporter UI에 `LogTimeline` 섹션을 추가해 hour JSON 기반 OTel 로그를 시간순/심각도별로 확인할 수 있게 했다.
- 인시던트 응답 구조에 `affectedServers`와 `postmortem`을 포함하고, Reporter 카드 및 Markdown/Text 다운로드 포맷에 반영했다.
- 연관 서버 링크는 `/dashboard?serverId=...` 쿼리로 연결되며, 대시보드에서 해당 서버 상세 모달이 자동 포커스된다.

### Phase 1 — 로그 타임라인 UI (난이도: ★☆☆)

기존 `public/data/otel-data/hourly/hour-XX.json` 로그를 Reporter 안에서 시각화한다. 추가 API 없음.

- [x] 1-1: Reporter 컴포넌트에 `LogTimeline` 서브 섹션 추가
  - 해당 인시던트 시간대 hour JSON에서 `logs[]` 추출
  - 타임스탬프 오름차순 정렬, severity(ERROR/WARN/INFO) 색상 구분
  - 서버 이름 기준 필터 드롭다운
- [x] 1-2: 로그 항목 클릭 시 상세 메시지 expand (accordion)
- [x] 1-3: 타임라인 기본 접힘(collapsed), "로그 타임라인 보기" 토글로 UX 노이즈 최소화

### Phase 2 — 연관 서버 자동 링크 (난이도: ★★☆)

GraphRAG의 `knowledge_relationships` traversal을 재활용해 동일 시간대 이상 서버를 자동으로 찾는다.

- [x] 2-1: Reporter Agent 프롬프트에 "동일 시간대 타 서버 이상 여부 교차조회" 지시 추가
  - 기존 `/api/ai/supervisor` 경로 활용 (추가 엔드포인트 불필요)
  - 결과: `affectedServers: [{ name, severity, metric, value }]`
- [x] 2-2: 보고서 UI에 "연관 서버" 섹션 추가
  - 서버명 + 심각도 배지 + 주요 메트릭 요약 1줄
  - 없으면 "연관 서버 없음" 표시 (빈 섹션 제거)
- [x] 2-3: 연관 서버 클릭 → `/dashboard?serverId=...` 경유로 해당 서버 상세 모달 자동 포커스

### Phase 3 — Postmortem MD 섹션 자동화 (난이도: ★☆☆)

Reporter Agent 출력 구조를 확장해 MD 복사/다운로드 시 Postmortem 섹션이 포함되도록 한다.

- [x] 3-1: Reporter Agent 프롬프트에 Postmortem 섹션 생성 지시 추가
  ```
  ## Postmortem
  ### 타임라인
  ...
  - [ ] (액션 아이템 2)
  ```
- [x] 3-2: UI에 Postmortem 탭 또는 섹션 추가 (기존 요약과 분리)
- [x] 3-3: MD 다운로드 파일명에 날짜 포함 (`incident-YYYYMMDD-HHMMSS.md`)
...
6. **T6-server-focus**: 연관 서버 링크 클릭 시 `/dashboard?serverId=...`로 이동하고 해당 서버 상세 모달이 자동 오픈
...
# AI Engine incident report contract 테스트
...
본 계획 범위는 `33d253543`, `bfe4f67d7`, `359fbc108` 커밋 계열과 후속 QA/production 검증으로 반영 완료되었다. 추가 작업은 Reporter 신규 기능이 아니라 보안·모달·UI UX 계획서에서 이어간다.
