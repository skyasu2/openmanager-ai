...
- [ ] **L1-1**: 메인 카드 에러 조건에 `!isGuestModalOpen` 추가 (1줄)
- [ ] **L1-2**: `Dialog.onOpenChange`에서 모달 닫힐 때 `setErrorMessage(null)` 호출
...
- [ ] **L2-1**: `LoginButtons.tsx` 취소 버튼 조건에 `loadingType === 'guest' || loadingType === 'email'` 추가 (1줄)
...
- [ ] **L3-1**: `login.constants.ts`에 `export type LoadingType` 추가
- [ ] **L3-2**: `LoginClient.tsx` 인라인 타입 → import로 교체
- [ ] **L3-3**: `LoginButtons.tsx` 로컬 타입 정의 제거 → import로 교체
- [ ] **L3-4**: `hooks/useLoadingMessages.ts` 로컬 타입 정의 제거 → import로 교체
...
- [ ] **L4-1**: `LoginButtons.tsx`에 3개 스타일 상수 직접 정의
- [ ] **L4-2**: `LoginButtonsProps`에서 `glassButtonBaseClass`, `providerOverlayClass`, `guestOverlayClass` 제거
- [ ] **L4-3**: `LoginClient.tsx`에서 3개 상수 정의 제거 및 props 전달 제거
...
- [ ] **L5-1**: `LoginButtonsProps.onEmail` 타입을 `(email: string) => Promise<boolean>`로 변경
- [ ] **L5-2**: `LoginClient.handleEmailLogin`이 성공/실패 boolean을 반환하도록 변경
- [ ] **L5-3**: `LoginButtons.handleEmailSubmit`을 async로 변경하고 성공 시 `setEmail('')` 실행
- [ ] **L5-4**: `<LoginButtons onEmail={handleEmailLogin}>` 형태로 전달해 void wrapper 제거
...
- [ ] **L6-1**: `_COOKIE_MAX_AGE_SECONDS` 삭제
- [ ] **L6-2**: `_PULSE_ANIMATION_DURATION_MS` 삭제
...
- [ ] **L7-1**: `login/error.tsx` style 객체 → Tailwind 클래스로 교체
- [ ] **L7-2**: reset 버튼 → `Button` 컴포넌트 또는 Tailwind 스타일 적용
