...
- `MISTRAL_SCALE_PLAN_CONFIRMED` 선행 조건은 **Scale plan 활성화**가 아니라 **비용 의존 없는 대체 경로 확정**으로 충족한다.
- Phase 4B/C는 production에서 LLM classifier를 필수로 만들지 않는다.
- 기존 프론트 regex는 BFF route-local deterministic classifier로 이관한다.
- `/api/ai/artifact-intent`는 deterministic match를 먼저 반환하고, LLM classifier는 기존처럼 `incident-report`/`monitoring-analysis` 후보 보강에만 사용한다.
- production에서 `MISTRAL_SCALE_PLAN_CONFIRMED=true`가 없어도 deterministic artifact intent는 동작해야 한다.
...
- type-only import users of `ChatArtifactIntent` / `ChatArtifactIntentReason`
