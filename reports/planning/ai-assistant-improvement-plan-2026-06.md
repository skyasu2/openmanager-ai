...
즉시 (단기, 1~2주)
  ...
  E-6  approval/incident 사용 여부    → 삭제 판단 선행 분석 (30분)
  E-1  command_vectors 잔여 이관+삭제 → DB 정리, 비가역 작업 전 검증

중기 (2~4주)
  E-2  knowledge_relationships 삭제   → 런타임 미사용 테이블 제거
  E-3  knowledge_base.embedding 제거  → 벡터 데이터 삭제, DB 크기 절감
  E-4  security_audit_logs retention  → DB 무한 증가 차단
  B-1  라우팅 회귀 감지 스크립트     → Harness 자동화 첫 단계
  C-1  orchestrator 중간층 분리       → 도메인 이식성 55% → 70%

장기 (1~2개월)
  E-5  Extension 스키마 migration     → Supabase advisor 경고 해소
