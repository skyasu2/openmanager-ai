...
| 진행 중 | G-1 | WONT-FIX 5개 항목 targeted remediation: 랭킹 advice, accordion 번호 목록, cross-metric ranking, server compare deterministic routing, summary markdown cleanup |
...
**작업**:
- [ ] 회귀 테스트 추가 및 실패 확인
- [ ] Fix 1: ASC 랭킹 advice 문구 정리
- [ ] Fix 2: accordion details 번호 목록 start offset 유지
- [ ] Fix 3: ranking-cross-metric 패턴과 deterministic current metric 조회 경로 추가
- [ ] Fix 4: server-compare orchestrator deterministic routing 보장
- [ ] Fix 5: summary 후보 markdown block marker 제거
- [ ] `npm run ci:local:quick` 및 AI Engine targeted/full 검증
- [ ] GitLab tag deploy 후 Production QA(P2, P3, P1) 기록

**검증 계획**:
- [ ] `cd cloud-run/ai-engine && npm test -- current-metrics-evidence-provider.test.ts`
- [ ] `cd cloud-run/ai-engine && npm test -- orchestrator-direct-routing.test.ts supervisor-stream-response.test.ts`
- [ ] frontend markdown renderer/unit test
- [ ] `npm run ci:local:quick`
- [ ] GitLab semver tag pipeline 확인
- [ ] Vercel production conversational AI QA targeted run 기록
...
진행 대상
 ...
 11. G-1  WONT-FIX 5개 targeted remediation → 진행 중. deterministic evidence/summary/frontend numbering 보강
