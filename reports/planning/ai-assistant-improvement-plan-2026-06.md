...
| 완료 | F-1 | P28/P29 결정적 evidence grammar 보강: 필터 메트릭과 응답 메트릭 분리, 단일 메트릭 상위+하위 동시 랭킹 |
| 완료 | G-1 | WONT-FIX 5개 항목 targeted remediation: 랭킹 advice, accordion 번호 목록, cross-metric ranking, server compare deterministic routing, summary markdown cleanup |
...
**Status: Conditional / Not triggered after 2026-06-06 recheck**

Analyst ReAct 루프 현재 maxSteps=5. 1차 개선 이후 step1 제거로 실질 최대 필요 step은 3~4로 줄었을 수 있음.

**작업**:
- [x] Langfuse trace에서 Analyst 경로 toolsCalled 배열 길이 분포 분석
- [x] 95th percentile step 수 확인 → 표본 부족으로 maxSteps=4 하향 미적용
- [x] before/after P95 지연 비교 → post-improvement `n=1`이라 P95 결론 보류

**결과**:
- 2026-06-06 재확인: `npm run langfuse:check -- --limit 100 --q supervisor --json`
- post-improvement 기준(2026-05-29 이후) supervisor trace 65건 중 Analyst trace는 `n=1`
- 해당 Analyst trace: `durationMs=6344`, provider `mistral`, `toolsCalled=[detectAnomalies, finalAnswer]`
- 판단: maxSteps 5 병목이나 P95 재상승 근거 없음. `maxSteps=5→4` 변경은 적용하지 않음
...
## Task F — 결정적 evidence grammar 보강

### F-1. P28/P29 크로스 메트릭 집계와 양방향 랭킹

**Status: Completed (2026-06-07)**

**현상**:
- P28: `"CPU 경고 서버들의 평균 디스크는?"` → 조건 메트릭(CPU)과 집계 메트릭(디스크)을 분리하지 못해 잘못된 집계로 흐를 수 있음.
- P29: `"디스크 기준 상위3+하위3"` → 단일 `rankOrder`만 보존해 상위/하위 동시 의도를 한쪽 랭킹으로 축소.

**작업**:
- [x] `filterConditions`를 추가해 대상 서버를 먼저 메트릭 조건으로 필터링한 뒤 별도 메트릭을 집계
- [x] `rankRange: top-bottom`을 추가해 단일 메트릭 랭킹에서 상위 N개와 하위 N개를 한 응답에 포함
- [x] P28/P29 회귀 테스트 추가

**검증**:
- [x] `cd cloud-run/ai-engine && npm test -- current-metrics-evidence-provider.test.ts` — 100 passed
- [x] `cd cloud-run/ai-engine && npm run type-check`
- [x] `cd cloud-run/ai-engine && npm test` — 148 files / 1651 tests passed
- [x] `npm run test:contract` — 24 passed

---

## Task G — WONT-FIX targeted remediation

### G-1. Verify Before Promotion 5개 항목 수정

**Status: Completed (2026-06-07)**

**현상**:
- `ranking-min-advice`: ASC(최저값·안정) 랭킹 응답에 경고성 확인 항목이 포함됨.
- `numbered-list-accordion-split`: summary/details가 별도 MarkdownRenderer로 렌더링되어 번호 목록이 details에서 1부터 재시작됨.
- `q-new106-ranking-cross-metric`: "메모리 상위N 서버들의 디스크 사용량"처럼 랭킹 대상 서버를 타깃으로 이종 메트릭을 조회하는 deterministic 경로가 없음.
- `server-comparison-deterministic-path`: `server-compare` evidence intent가 감지돼도 orchestrator에서 deterministic provider를 보장하지 않아 LLM fallback 가능성이 남음.
- `summary-block-markdown-heading-hr-code-fence`: summary 추출 후보에 heading/hr/code fence 라인이 섞여 프론트엔드 summary가 일반 텍스트처럼 보일 수 있음.

**계약 (Contract)**:

| 항목 | 입력 | 기대 출력 |
|------|------|-----------|
| ranking-min-advice | ASC metric ranking 또는 top-bottom 하위 섹션 | 경고/조치성 문구 대신 안정적 수치 문구 또는 advice 생략 |
| numbered-list-accordion-split | summary가 번호 목록 1..N, details가 이어지는 번호 목록 | details 첫 `<ol>`이 `start=N+1`로 렌더링 |
| q-new106-ranking-cross-metric | "메모리 상위 3개 서버들의 디스크 사용량" | 메모리 랭킹 서버 ID를 대상으로 디스크 current metric deterministic evidence 반환 |
| server-comparison-deterministic-path | 명시적 서버 비교 질의 | orchestrator direct routing이 provider `deterministic`을 강제 |
| summary-block-markdown-heading-hr-code-fence | heading/hr/code fence 포함 응답 | summary에는 heading marker, hr, code fence marker가 제거된 plain text만 포함 |

**작업**:
- [x] 회귀 테스트 추가 및 실패 확인
- [x] Fix 1: ASC 랭킹 advice 문구 정리
- [x] Fix 2: accordion details 번호 목록 start offset 유지
- [x] Fix 3: ranking-cross-metric 패턴과 deterministic current metric 조회 경로 추가
- [x] Fix 4: server-compare orchestrator deterministic routing 보장
- [x] Fix 5: summary 후보 markdown block marker 제거
- [x] Production QA에서 확인된 dangling ordered-list marker 경계 보강
- [x] `npm run ci:local:quick` 및 AI Engine targeted/full 검증
- [x] GitLab tag deploy 후 Production QA(P2, P3, P1) 기록

**검증 계획**:
- [x] `cd cloud-run/ai-engine && npm test -- current-metrics-evidence-provider.test.ts orchestrator-direct-routing.test.ts supervisor-stream-response.test.ts`
- [x] `cd cloud-run/ai-engine && npm run type-check`
- [x] `cd cloud-run/ai-engine && npm test`
- [x] `npm run test:dom -- AIWorkspaceMessage.detail.test.tsx MarkdownRenderer.test.tsx`
- [x] `npm run type-check`
- [x] `npm run lint`
- [x] `npm run test:quick`
- [x] `npm run test:contract`
- [x] `npm run ci:local:quick`
- [x] GitLab tag pipeline `2582574231` (`v8.12.106`) success
- [x] Production QA `QA-20260607-0674` success, WONT-FIX 32 -> 27

**결과**:
- 배포: `v8.12.106` (`c998cc7b8`), GitLab pipeline `2582574231` success.
- QA: `QA-20260607-0674`, targeted release-facing, 6/6 pass, completed improvements 5개.
- 비용/사용량: `npm run check:usage:vercel` checked/normal, billed `0.0000 USD`.
...
진행 대상
 ...
 11. G-1  WONT-FIX 5개 targeted remediation → 완료. `QA-20260607-0674` production targeted QA pass
 12. D-2  Analyst maxSteps 하향 검증     → 2026-06-06 재확인 n=1, 병목 근거 없어 보류
