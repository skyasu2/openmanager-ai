...
**Last Updated**: 2026-05-22 KST (v8.12.5 QA 후속 H-2 peak metric deterministic response local implementation 완료)
...
|------|----------|--------|-------|
| AI 품질 개선 (grounded KRL QA·intentFrame 관찰·Z.AI 안정성) | High | In Progress (tracking) | 2026-05-22 v8.12.5 Playwright MCP 신규 6문항 QA(`QA-20260522-0559`) 완료. 4 PASS / 2 PARTIAL. **신규 발견**: I-1 서버 1:1 비교 쿼리가 일반 대화 경로로 라우팅되어 수치 오류 발생(82%→92%), I-2 심층 원인 분석이 generic 결과 반환(도메인 특성 미반영), I-3 Reporter 경고 기준 대시보드 불일치(1대 vs 4대). I-1은 `710c6165d` failing test 선행 후 local implementation 완료(AI Engine type-check/full test PASS). I-2는 `6ce60db4e` failing test 선행 후 supervisor/Analyst prompt에 backup DISK·Redis MEM 역할별 RCA 힌트를 반영해 local implementation 완료(AI Engine type-check/full test PASS). I-3은 Reporter UI/다운로드 문구에서 임계값 초과 서버와 의존성 포함 영향 범위를 구분하도록 local implementation 완료. H-1은 `8f12a0b30` failing test 선행 후 capacity forecast artifact bypass, 목표 threshold=90 파싱, 지정 서버 target fallback을 local implementation 완료. H-2는 `d09af3f46` failing test 선행 후 peak metric evidence를 deterministic fallback 응답으로 고정해 local implementation 완료. 이전 상태: v8.12.4 5문항 QA → v8.12.5 3개 이슈 수정 배포 완료. v8.12.5 GitLab pipeline `2545042099` success. 잔여: intentFrame 실측 적중률, Task F 최종 판정. Task E SDD Approved. 상세: [ai-quality-improvement-plan-2026-05.md](ai-quality-improvement-plan-2026-05.md) |
...
| Evidence Provider 라우팅·응답 품질 개선 | P2 | 2026-05-22 v8.12.5 QA(QA-20260522-0558/0559) 발견. H-1 `monitoringCapacityForecastEvidenceProvider` 불일치 라우팅, H-2 `monitoringPeakMetricEvidenceProvider` 응답 내용 부실 모두 local implementation 완료. **신규 Task I**: I-1 서버 1:1 비교 쿼리 deterministic 경로, I-2 심층 분석 도메인 특성 힌트, I-3 Reporter 기준 명시는 모두 local implementation 완료. 상세: [ai-quality-improvement-plan-2026-05.md](ai-quality-improvement-plan-2026-05.md) Task H/I |
...
- [x] KRL corpus cap 상향 및 security/architecture seed 보강
  - Supabase 공식 Free plan DB 500MB 한도 대비 7건 텍스트 seed 추가는 사용량 문제가 낮다고 판단해 `target 72 / hard 80`으로 governance를 상향했다.
  - live Supabase에 `security +4`, `architecture +3`을 upsert해 총 67건으로 확장했다.
  - command 비율은 25/67=37.3%로 기존 41.7%보다 낮아졌고, `security=5`, `architecture=8` 모두 신규 target 범위 안이다.
  - 검증: policy spec, seed dry-run, `supabase:rag:smoke` 19/19 PASS, `rag:analyze` 전체 PASS. 상세: [archive/krl-corpus-cap-expansion-plan.md](archive/krl-corpus-cap-expansion-plan.md)
...
- [x] AI Assistant 코드/문서 개선 C1~C3/D4 완료
  ...
  - C2: `useAIChatCore.ts` 607→578 (`chat-core-send-plan.ts` 추출)
  - C3: `useHybridAIQuery.ts` 704→523 (`async-result-message.ts` 추출)
  - D4: [ai-hooks-map.md](../../docs/reference/architecture/ai/ai-hooks-map.md) 신규 작성으로 9개 AI hook entry map 정리
...
- [x] AI 어시스턴트 이식성 개선 T1~T4 완료
  - `ChatArtifact`를 도메인 중립 베이스 계약으로 정리하고, 모니터링 5종 유니온은 monitoring domain으로 이동했다.
  - Artifact renderer/executor registry 전환으로 frontend hardcoded switch 경계를 제거했다.
  - 참조 문서 [domain-portability.md](../../docs/reference/architecture/ai/domain-portability.md)를 추가해 backend `AssistantDomain`, frontend artifact registry, 포팅 checklist, stock/code-review shell 예시를 정리했다.
  - 검증: `type-check`, `lint`, `test:quick`, `test:contract`, `build:ci`, docs checks PASS. 상세: [archive/ai-portability-improvement-plan.md](archive/ai-portability-improvement-plan.md)
