...
| **Provider Quota 재배치 (Q1)** | **High** | **Approved** | **Q0 완료**: `gpt-oss-120b` 정책을 `fallback/enabled/green`으로 보정하고 model-aware quota + built-in Cerebras fallback 후보로 등록. `DEFAULT_CEREBRAS_MODEL`은 아직 `llama3.1-8b` 유지. **Q1 P1**: Orchestrator를 Groq-first→Cerebras/Mistral/Z.AI/Groq-last로 재배치하고, forced routing 전 `decomposeTask()` 선행 낭비를 제거. Metrics-heavy 최악 Groq 2~7회 → 0~4회 목표. **2026-05-26**: `DEFAULT_CEREBRAS_MODEL`→`CEREBRAS_GPT_OSS_MODEL_ID` 전환 여부 재확인. 상세: [provider-quota-rebalance-plan.md](provider-quota-rebalance-plan.md) |
