...
**Last Updated**: 2026-04-18 KST (OTel Phase 3-B AZ3 Redis slice 승인)
...
|------|----------|--------|-------|
| OTel topology improvement - Phase 3-B AZ3 Redis | Medium | approved | `cache-redis-dc1-03`를 AZ3 inventory로 추가. 범위는 `resource-catalog`, `server-registry`, 24개 hourly, `timeseries`, `otel-fix`, `otel-verify`에 한정. `storage-nfs-dc1-02`와 precomputed-state 재생성은 제외. |
...
| OTel 토폴로지 개선 (Phase 3) | Medium | 계획서: [otel-topology-improvement-plan.md](otel-topology-improvement-plan.md). Phase 3-A `lb-haproxy-dc1-03` 완료. 현재는 Phase 3-B `cache-redis-dc1-03`만 active slice이며, `storage-nfs-dc1-02`와 필요 시 precomputed-state 재생성은 backlog. |
