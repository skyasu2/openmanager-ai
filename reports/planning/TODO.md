...
**Last Updated**: 2026-05-20 KST (Redis retention decision gate added)
...
|------|----------|--------|-------|
| Redis 사용 현황 정비 (사문화 코드·Job Queue 단일 의존성·문서 불일치) | Medium | Draft | 2026-05-20 코드 전수 조사. Redis는 과거 long-running Job Queue 용도만 남은 상태가 아니며 Vercel/Cloud Run rate limit, Guest PIN 방어, `system:running`, Cloud Run quota/cache, Langfuse usage guard에도 사용 중이다. R-0 inventory에서 `/api/ai/jobs*`가 `useHybridAIQuery`/`useAsyncAIQuery`/Cloud Run job worker에 아직 연결된 활성 경로임을 확인했다. R-3 Job Queue 503 보강은 완료되어 Redis client 없음/primary write 실패가 503으로 정렬됐다. 다음 결정은 async Job Queue 유지/제거이며, 사문화 2건(resumable/CB store)은 ai-assistant-design-cleanup-plan Task 1-C·3-C에 통합한다. 상세: [redis-usage-cleanup-plan.md](redis-usage-cleanup-plan.md) |
| AI 어시스턴트 설계 정비 (resume 불일치·CB 누락·라우팅 중복 등) | High | Draft | 정적 분석으로 발견한 Critical 2건·Major 4건·Minor 4건. 2026-05-20 Task 1-B를 Critical→Major/P2로 재분류(Cloud Run multi-provider rotation이 실질 fault tolerance 담당). 2026-05-20 Codex가 Vercel AI SDK v6 공식 기준과 프로젝트 Free Tier 원칙에 맞춰 계약/테스트 시나리오를 보강했다. 적용 기준은 Vercel-only 수렴이 아니라 `AI SDK UI/protocol compatibility + Cloud Run AI Engine runtime ownership`이다. 1-A는 2-B의 legacy 202 redirect 제거에 종속되는 안전망이고, 1-B는 stream fallback/header/OPEN-state test를 포함하는 M 규모 작업으로 재분류했다. `stream/v2` GET 처리·distributed CB export·QueryClassifier export 결정을 마치면 Approved 전환 가능. 상세: [ai-assistant-design-cleanup-plan.md](ai-assistant-design-cleanup-plan.md) |
