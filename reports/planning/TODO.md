...
**Last Updated**: 2026-05-17 KST (AI 아티팩트 렌더러 T2a 전환)
...
| AI 어시스턴트 이식성 개선 — 프론트엔드 아티팩트 레지스트리 전환 | Low | In Progress (Codex) | 백엔드는 이미 도메인 플러그인 구조가 충분하므로 프론트엔드 하드코딩 제거에 집중. T2a 렌더러 등록 API와 `ArtifactRendererHost` switch 제거 완료, 다음 범위는 T1 타입 제너릭화 또는 T3 실행기 레지스트리 전환. 상세: [ai-portability-improvement-plan.md](ai-portability-improvement-plan.md) |
---
