...
**Last Updated**: 2026-05-08 KST (`Dependency hygiene Task 2 AI Engine patch batch (Codex)`)
...
> **권장 실행 순서**: ① Dependency & config hygiene Task 3 — root low-risk patch batch
...
| Task | Priority | Notes |
|------|----------|-------|
| Dependency & config hygiene 정리 | High | Approved. Task 1 local install tree cleanup과 Task 2 AI Engine patch/minor security batch 완료. 다음은 root low-risk patch batch와 `shadcn` CLI dependency 결정. 상세: [dependency-config-hygiene-plan.md](dependency-config-hygiene-plan.md) |
