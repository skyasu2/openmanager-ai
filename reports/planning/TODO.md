...
**Last Updated**: 2026-05-22 KST (v8.12.4 Vercel 프로덕션 QA — AI 어시스턴트 5문항 평가 + 신규 이슈 식별)
...
|------|----------|--------|-------|
| AI 품질 개선 (grounded KRL QA·intentFrame 관찰·Z.AI 안정성) | High | In Progress (tracking) | 2026-05-22 v8.12.4 Vercel 프로덕션 QA 신규 5문항 평가: Q1(이상 징후 탐지) 양호 — 서버 식별 정확, 추세 구분 정상. Q2(용량 예측 "언제 90% 넘을까") 라우팅 실패 — 이상감지로 잘못 분류, 예측값 없음. Q3(장애 원인 분석) 양호 — 근거·권고 구체적, jstack grep 패턴 소수 오류. Q4(에러 로그 TOP 3) 부분 성공 — 도구 제약 투명 공개, 결과 미완성. Q5(AZ 부하 비교) 실패 — 1차 "AI job completed without a substantive response body" 에러, 재시도는 AZ 분리 없이 전체 요약 반환 + MEM/Disk N/A. 신규 개선 항목: (A) routing-policy에 "언제 X% 넘을까/용량 예측" capacity planning intent 추가. (B) AZ 그룹별 집계 지원(filterServers AZ 파라미터). 배포 완료: ClarificationDialog overflow fix·supervisor-prompt 수치 자기모순 방지 포함 v8.12.4 Vercel/Cloud Run 배포 완료(commit 4c79450c6, GitLab pipeline 2544921351 success). Task E 세션 메모리 확장은 Supabase 로그인 사용자만 `sessionOwnerKey`를 전달하고 guest/API key/dev/test는 기존 Redis 경로를 유지하는 계약으로 Approved 상태다. 상세: [ai-quality-improvement-plan-2026-05.md](ai-quality-improvement-plan-2026-05.md) |
