...
**Last Updated**: 2026-05-22 KST (v8.12.5 AI 품질 3개 이슈 수정 배포 — capacity forecast routing, AZ 비교 evidence, OTel CI fix)
...
|------|----------|--------|-------|
| AI 품질 개선 (grounded KRL QA·intentFrame 관찰·Z.AI 안정성) | High | In Progress (tracking) | 2026-05-22 v8.12.4 5문항 QA → v8.12.5 3개 이슈 수정 배포 완료. **Q2 fix**: `CAPACITY_FORECAST_EXCLUSION_PATTERN` 추가로 "언제 90% 넘을까" 쿼리가 artifact가 아닌 Cloud Run prediction 경로로 정상 라우팅. **Q5 fix**: supervisor-prompt에 DC1-AZ1/AZ2 비교 시 `metric='all', groupBy='location'` 필수 few-shot 예시 추가, `monitoringLocationLoadBalanceEvidenceProvider`/`monitoringCapacityForecastEvidenceProvider` 결정적 evidence path 신규 추가(commit `53a63b4d9`). CI OTel 데이터 로딩 문제 수정(commit `1392cfc15`). v8.12.5 GitLab pipeline `2545042096` success, Vercel+Cloud Run 배포 완료. 잔여 미관찰: Q4(에러로그 TOP3) 도구 제약 재현 재평가, intentFrame 실측 적중률. Task E 세션 메모리 확장은 Supabase 로그인 사용자만 `sessionOwnerKey`를 전달하고 guest/API key/dev/test는 기존 Redis 경로를 유지하는 계약으로 Approved 상태다. 상세: [ai-quality-improvement-plan-2026-05.md](ai-quality-improvement-plan-2026-05.md) |
