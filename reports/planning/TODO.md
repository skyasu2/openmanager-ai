...
**Last Updated**: 2026-05-08 KST (`Dependency hygiene Task 3 root TypeScript patch (Codex)`)
...
> **권장 실행 순서**: ① Dependency & config hygiene Task 4 — `shadcn` CLI dependency 결정
...
| Task | Priority | Notes |
|------|----------|-------|
| Dependency & config hygiene 정리 | High | Approved. Task 1 local install tree cleanup, Task 2 AI Engine patch/minor security batch, Task 3 root TypeScript/@types patch 완료. Storybook/Playwright/Vitest patch는 registry/peer alignment 문제로 보류. 다음은 `shadcn` CLI dependency 결정. 상세: [dependency-config-hygiene-plan.md](dependency-config-hygiene-plan.md) |
