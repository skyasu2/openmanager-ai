...
**Last Updated**: 2026-05-10 KST (`dependency security audit triage 진행`)
...
| Task | Priority | Notes |
|------|----------|-------|
| Dependency security audit follow-up | P1 | Root production audit는 `next@16.1.6` high 1건만 잔류. 같은-major fixed release(`next@16.1.7` 또는 `16.2.3+`)가 registry에 제공되면 즉시 업데이트하며, `npm audit fix --force`의 Next 15 downgrade는 금지. Root full audit의 잔여 dev critical은 `commit-and-tag-version -> handlebars` 체인이라 release tooling 교체/대체 여부를 별도 판단. AI Engine audit은 `promptfoo@0.121.3` + scoped overrides로 low-only까지 축소했고, 남은 AI Engine production low는 Google Cloud logging transitive upstream 추적. |
