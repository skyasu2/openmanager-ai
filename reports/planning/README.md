...
대형 리팩터는 리뷰 가능성과 rollback 범위를 기준으로 2~3개 논리 커밋으로 나눈다.

| 커밋 | 포함 범위 | 기준 |
|------|-----------|------|
| `test(spec):` | 계약·회귀 테스트 | 신규 기능, 대규모 리팩터링, API/AI/auth/monitoring 계약 변경이면 먼저 작성 |
| `refactor:/feat:/fix:` | 실제 구현 | 파일 분리, 역할 경계 변경, 런타임 동작 변경을 한 커밋에 집중 |
| `docs:/test(qa):` | 문서·계획·QA 기록 | 구현 검증 결과, TODO/plan archive, QA record만 별도 정리 |

분할이 필요한 경우:
- 파일·모듈 경계 변경과 문서/QA 정리가 함께 있는 경우
- 테스트 계약 추가와 구현 변경이 모두 있는 경우
- DB migration, 배포 설정, 런타임 경로 변경처럼 rollback 판단 단위가 다른 경우
- 변경 파일이 많아 리뷰 초점이 흐려지는 경우

분할하지 않아도 되는 경우:
- 단일 파일 소규모 리팩터링 또는 단순 문구 수정
- 기계적 rename/import 정리처럼 커밋을 나누면 오히려 diff 추적이 어려운 경우
- pre-commit formatter가 만든 동일 범위의 후속 정리
...
_Last Updated: 2026-05-15 (대형 리팩터 커밋 분할 기준 추가)_
