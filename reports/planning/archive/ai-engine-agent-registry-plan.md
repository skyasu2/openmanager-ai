...
- 상태: Completed
- 작성일: 2026-05-06
- TODO.md 연결: Recent Completed #306 > AI Engine domain-owned agent registry
- 선행 완료:
  - [ai-assistant-portable-core-domain-pack-plan.md](ai-assistant-portable-core-domain-pack-plan.md) — core/domain/adapter 분리
  - [ai-engine-supervisor-domain-wiring-plan.md](ai-engine-supervisor-domain-wiring-plan.md) — supervisor host 배선
  - [ai-engine-sdk-decoupling-plan.md](ai-engine-sdk-decoupling-plan.md) — SDK execution 경계
...
- [x] core contract: `AgentRole`, `AgentRoleRegistry`, `AssistantDomain.agentRoles?` 타입을 사용할 수 있다.
- [x] monitoring manifest: `monitoringDomainPack.agentRoles.listRoles()`가 기존 7개 role id/name/runtimeConfigKey를 모두 제공한다.
- [x] drift guard: monitoring role `runtimeConfigKey`는 `AGENT_CONFIGS`의 key와 1:1로 해석된다.
- [x] legacy regression: `AgentFactory.create('nlq')`, `getAvailableTypes()`, `getAvailabilityStatus()`의 기존 surface가 유지된다.
- [x] domain factory: `AgentFactory.createByDomain('nlq', monitoringDomainPack)`은 기존 NLQ agent와 같은 config name을 반환한다.
- [x] domain factory fallback: registry 없음/unknown role/runtime binding 없음은 예외 없이 `null` 또는 legacy-compatible fallback으로 처리된다.
- [x] sample portability: sample domain은 monitoring import 없이 2개 role을 등록하고 registry 조회 smoke를 통과한다.
...
- [x] Task 0 — failing specs: AgentRole 계약, monitoring manifest drift guard, factory fallback, sample registry smoke
- [x] Task 1 — `core/assistant-runtime/types.ts`에 `AgentRole` / `AgentRoleRegistry` 추가
- [x] Task 2 — `domains/monitoring/agent-roles.ts` 작성 (기존 7개 agent id/name/description/pattern/runtimeConfigKey 이관)
- [x] Task 3 — `monitoringDomainPack`에 `agentRoles` 필드 추가하고 domain contract test 통과
- [x] Task 4 — `AgentFactory.createByDomain()` 추가 (기존 `create(type)` 유지, registry 없음/unknown/binding 없음 semantics 반영)
- [x] Task 4.5 — `agent-configs.ts` description/matchPatterns 파생 및 `orchestrator-factory.ts` helper drift 제거
- [x] Task 5 — sample domain에 agent role 2개 추가 + registry smoke test
...
이 항목들까지 완료되면 "다른 도메인 붙이기 = domain pack + agent roles + data adapter만 작성"
수준이 된다.

## 검증 결과

- Task 0 failing specs: targeted AI Engine tests `4 files / 7 failed, 33 passed`
- Targeted implementation tests: `4 files / 40 tests passed`
- AI Engine type-check: pass
- AI Engine full test: `102 files / 1053 tests passed`
- root `type-check`: pass
- root `lint`: pass
- root `test:quick`: pass
- root `test:contract`: `20/20 passed`
- docs: `docs:budget`, `docs:ai-consistency`, `git diff --check` pass
...
- 2026-05-06: SDD 순서대로 Task 0 failing specs 커밋 후 구현. Monitoring domain이 7개
  role manifest를 소유하고, `agent-configs.ts` description/matchPatterns는 manifest에서 파생된다.
  `AgentFactory.createByDomain()`은 monitoring role runtime binding, registry 없음 fallback,
  unknown role, runtime binding 없음을 deterministic하게 처리한다. Targeted/full/local root gate
  통과 후 plan을 Completed로 archive 이동.
