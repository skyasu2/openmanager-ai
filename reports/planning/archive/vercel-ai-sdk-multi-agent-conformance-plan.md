...
**보류 이유 — 아키텍처 구조적 제약 (재검토 전 필독)**
> ⚠️ AI SDK subagent-as-tool 패턴은 **에이전트 런타임이 단일 서버에서 동작한다는 전제**를 가진다.
> 현재 구조는 Vercel(BFF) + Cloud Run(AI Engine)으로 분리되어 있어, AI SDK의 `tool()` 기반 subagent compose가
> 같은 runtime context에서 이루어지지 않는다.
> 이 구조적 제약을 해소하려면 Cloud Run → Vercel로 에이전트 로직을 이전하거나 양쪽을 연결하는 브리지 레이어가 필요하며,
> 복잡도 증가 대비 즉시 ROI가 없다. **아키텍처 분리 정책이 바뀌지 않는 한 재착수하지 않는다.**
- 추가: subagent는 latency와 LLM 호출 수를 늘린다.
- 추가: 현재 provider Free Tier에서는 default 병렬 subagent가 RPM 병목을 키울 수 있다.
...
**보류 이유 — 아키텍처 구조적 제약 (재검토 전 필독)**
> ⚠️ `createAgentUIStreamResponse`는 **LLM 호출부터 UI 응답까지 단일 서버에서 스트림을 소유한다는 전제**를 가진다.
> 현재 구조에서 스트림은 Cloud Run이 생성하고 Vercel API route가 proxy한다(이중 프록시).
> Vercel에서 AI SDK native UI stream을 originate하려면 LLM 호출 자체를 Vercel로 이전해야 하며,
> 이는 Vercel 함수 타임아웃/메모리 제약과 충돌한다. **아키텍처 분리 정책이 바뀌지 않는 한 재착수하지 않는다.**
...
**보류 이유 — 아키텍처 구조적 제약 (재검토 전 필독)**
> ⚠️ AI SDK DevTools는 AI SDK 호출을 단일 훅으로 관찰한다.
> 현재 구조는 AI SDK 호출이 Vercel(엔티티 추출, artifact intent)과 Cloud Run(실제 LLM 호출) 양쪽에 분산되어 있어,
> 단일 DevTools 세션으로 전체 파이프라인을 추적하기 어렵다.
> **아키텍처 분리 정책이 바뀌지 않는 한 재착수하지 않는다.**
...
**2026-05-16 적용 결과**
- `portable-core-ai-sdk-mock-provider.bench.test.ts`를 추가해 `MockLanguageModelV3` 기반 비용 0원 benchmark를 고정했다.
- metric lookup(single)과 incident report escalation(multi)의 route/plan snapshot을 고정했다.
- `generateText` forced-routing surface와 `streamText` agent-stream surface가 동일한 `Metrics Query Agent` loop ceiling(`maxSteps=4`, `maxOutputTokens=2048`, `sdkMaxRetries=0`)을 쓰는지 검증한다.
- mock provider `doGenerate`/`doStream` 호출 횟수를 각각 1회로 고정해 SDK retry amplification이 없음을 보장한다.
...
### 2026-05-16 4차 optional O4 검증 결과

| Command | Result |
|---------|--------|
| `cloud-run/ai-engine npm run test -- --run portable-core-ai-sdk-mock-provider portable-core-route-retrieval portable-core-stream-contract` | PASS, 3 files / 5 tests |
| `cloud-run/ai-engine npm run test -- --run portable-core-ai-sdk-mock-provider orchestrator-execution.timeout orchestrator-routing orchestrator-agent-stream` | PASS, 4 files / 55 tests |
| `cloud-run/ai-engine npm run type-check` | PASS |
