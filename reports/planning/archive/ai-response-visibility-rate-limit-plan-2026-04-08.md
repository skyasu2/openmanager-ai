> Owner: project
> Status: Complete — `handoff persistence contract`, `429 UX source-hardening`, `Job Queue agent-path`, `/api/ai/jobs` POST gateway limiter alignment, frontend AI gateway session-aware limiter identity, daily-limit semantics, Cloud Run forwarded identity, retry limiter alignment, Cloud Run jobs read/write limiter split, Cloud Run daily semantics alignment, Cloud Run supervisor health limiter split, Cloud Run read-only window alignment, write bucket 재평가까지 종료. 이 문서는 구현/결정 로그로 유지.
> Doc type: Plan
...
- 상태: **Complete** — AnalysisBasisBadge 중심 visibility 개선과 `handoff persistence contract`, `429 UX source-hardening`, `Job Queue agent-path`, `/api/ai/jobs` POST gateway limiter alignment, frontend AI gateway session-aware limiter identity, daily-limit semantics, Cloud Run forwarded identity, retry limiter alignment, Cloud Run jobs read/write limiter split, Cloud Run daily semantics alignment, Cloud Run supervisor health limiter split, Cloud Run read-only window alignment, write bucket 재평가까지 종료됐다. 이 문서는 구현/결정 로그로 유지한다.
...
### 이번 slice 완료 결과 (`2026-04-18`, Cloud Run read-only window alignment)

- `GET /api/ai/supervisor/health`와 `GET /api/jobs/:id`, `GET /api/jobs/:id/progress` read-only bucket은 이제 `120/min`을 사용한다.
- provider 비용을 직접 태우는 `supervisor` write `10/min`, `jobs/process` write `5/min`, `daily=100` semantics는 그대로 유지된다.
- read-only direct polling 및 health/smoke 경로가 `60/min` 경계에서 false 429를 내던 정책 drift를 줄였다.
- 남은 backlog는 write bucket 수치 자체를 다시 평가하는 작업이다.

### 최종 결정 (`2026-04-18`, write bucket 재평가)

- `supervisor 10/min`, `jobs/process 5/min`, `daily 100` write bucket은 현재 그대로 유지한다.
- 근거:
  - frontend와 Cloud Run write 경로는 이미 같은 minute/daily semantics로 정렬돼 있다.
  - read-only 경로는 별도 bucket과 `120/min` 완화로 false 429를 줄였다.
  - 추가 상향은 무료 티어 방어만 약화시키고, 현재 확보된 production evidence 범위에서는 사용자 체감 이득이 불충분하다.
- 후속 원칙:
  - write bucket 변경은 production 429 evidence가 새로 쌓일 때만 재개
  - 현재 값은 회귀 테스트로 고정한다
