...
- TODO.md 위치: `Backlog (완료 이력)`
- 구현 커밋: `767acd026 feat(ai): delegate guard warnings to llm`
- GitLab pipeline: `2530042325` success
- 완료된 범위: Phase 1/2 백엔드 계약(T1~T3, T6~T9), Phase 3 문서 갱신(T10~T16)
- 채택한 UI 계약: 별도 metadata/banner가 아니라 assistant 응답 본문/SSE `text_delta` 앞에 경고 문구를 prepend한다.
- 배포 완료: `v8.11.160` release/tag pipeline `2530076991` success. Vercel production `/api/version=8.11.160`, Cloud Run `/health.version=8.11.160`, revision `ai-engine-00477-fxw` 100% traffic.
...
- [x] T10. `docs/architecture/01-system-overview.md` Mermaid
  - `Supervisor / Orchestrator / Agents` → `Direct Router / 5 Specialist Agents`
  - 오프도메인 경고 위임 흐름 반영

- [x] T11. `docs/architecture/02-runtime-architecture.md` Mermaid
...
- [x] T12. `docs/reference/architecture/system/system-architecture-current.md` Mermaid
  - `7 Agents + Orchestrator` → `5 Specialist Agents + Direct Router`
  - `AI 실행 컴포넌트 8개` 수치 수정

- [x] T13. `docs/reference/architecture/ai/ai-engine-architecture.md` Mermaid (2개)
...
- [x] T14. **요청 분기 전체 흐름 (신규)** — `docs/reference/architecture/ai/ai-engine-architecture.md` 또는 `02-runtime-architecture.md`에 추가
  ```
...
- [x] T15. **Provider 선택 Round-Robin 흐름 (신규)** — `docs/adr/adr-006-llm-provider-load-balancing.md`에 추가
  ```
...
- [x] T16. **오프도메인·보안 처리 결정 트리 (신규)** — 신규 `docs/reference/architecture/ai/request-guard-policy.md` 생성
  ```
...
```
Phase 1 (T1~T6 완료) → Phase 2 (T7~T9 완료) → Phase 3 다이어그램 갱신 (T10~T16 완료) → Phase 4 (T17 완료)
```
...
1. T10~T16 문서 갱신 완료.
2. plan `Status`를 `Completed`로 전환하고 archive 이동.
3. `v8.11.160` release/tag 배포 완료.
