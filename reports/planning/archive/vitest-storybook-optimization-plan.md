...
> Status: Completed
> Doc type: Plan
> Last reviewed: 2026-05-24 (bundlemon warn-first → blocking 승격 불필요 결정)
...
- [x] 중간 sanity check 기록 (2026-05-19 KST)
  - `npm run bundle:budget` PASS
  - gzip 기준 JS chunks group: `1.38MB < 2MB`
  - gzip 기준 CSS chunks group: `61.94KB < 250KB`
  - 최대 단일 JS chunk: `143.29KB < 300KB`
  - 최대 단일 CSS chunk: `30.89KB < 120KB`
  - 판단: 2026-05-18 follow-up 다음날 확인이므로, 1~2주 관측 후 blocking 승격 여부 결정은 아직 대체하지 않는다.
- [x] 후속 관측 기록 (2026-05-21 KST, v8.11.194)
  - `npm run bundle:budget` PASS
  - gzip 기준 JS chunks group: `1.45MB < 2MB`
  - gzip 기준 CSS chunks group: `61.69KB < 250KB`
  - 최대 단일 JS chunk: `143.29KB < 300KB`
  - 최대 단일 CSS chunk: `30.89KB < 120KB`
  - 판단: v8.11.194 후속 관측도 예산 안에 있으므로 warn-first 유지. blocking 승격 여부는 기존 2026-05-30 전후 관측 기준으로 판단한다.
- [x] 후속 관측 기록 (2026-05-24 KST, v8.12.19)
  - `npm run bundle:budget` PASS
  - gzip 기준 JS chunks group: `1.43MB < 2MB`
  - gzip 기준 CSS chunks group: `61.95KB < 250KB`
  - 최대 단일 JS chunk: `143.27KB < 300KB`
  - 최대 단일 CSS chunk: `31.12KB < 120KB`
  - 판단: v8.12.19 기준 production build와 bundle budget 모두 PASS. JS group은 2026-05-21 관측보다 낮고 CSS group은 동일 범위라 warn-first 유지가 타당하다. blocking 승격 여부는 기존 2026-05-30 전후 관측 기준으로 최종 판단한다.
- [x] 2026-05-24 결정: warn-first 유지 (blocking 승격 불필요). 5회 연속 PASS, 수치 안정적. regression 실제 발생 시 별도 hotfix로 처리.
