...
- 2026-05-05: RAG OTel/SSOT 문서 경로 hallucination 잔여 이슈를 재점검했다. `QA-20260505-0410`에서 운영 QA는 회복됐고, clarification skip, RAG tool forcing, insufficient-evidence fallback, grounded path extraction 테스트가 이미 존재한다. 다만 `/opt/otel` 및 `/path/to/OpenManager` placeholder 재발 방지를 portable core 완료 기준에 명시해 Task 7에서 다시 검증한다.
