...
**분석 기준일**: 2026-05-05
**현재 구현 기준**: OpenManager AI v8.11.97, Vercel Frontend + Cloud Run AI Engine
...
- `/api/ai/ask` facade, planner shadow, `MonitoringFactPack`, provider freshness 같은 구현 세부 변경은 먼저 정본 문서에 반영하고, 장기 방향성이 바뀔 때만 이 문서의 비교 결론을 갱신합니다.
- artifact workspace/schema registry, facade opt-in 확대, provider-native reasoning policy가 완료되어 핵심 결론이 정본 문서에 흡수되면 이 문서는 `docs/archived/` 이동 후보가 됩니다.
...
- M1~M7과 AI Streaming UI S1~S3가 완료되어 단순 read-only contract 단계를 넘어 **wrapper facade + deterministic fact boundary + stream step visibility** 단계에 들어갔다.
