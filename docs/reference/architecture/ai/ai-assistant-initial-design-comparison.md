...
**분석 기준일**: 2026-05-03
**현재 구현 기준**: OpenManager AI v8.11.87, Vercel Frontend + Cloud Run AI Engine
...
- 현재 구현은 여전히 **Option A 개선 중간 단계**지만, 2026-05-03 작업으로 M1~M5c가 완료되어 단순 read-only contract 단계를 넘어 **측정 가능한 planner shadow 단계**에 들어갔다.
...
- M6 `/api/ask` wrapper-only facade까지 완료됐고, `MonitoringFactPack`/eval guard(M7), AI Streaming UI S1~S3는 아직 남은 제품/분석 목표다.
...
| 개선 | 방법 | 현재 구현 상태 |
|------|------|----------------|
| BFF surface 수렴 | `/api/ask` facade를 목표로 삼되, 먼저 기존 streaming/job/artifact route가 같은 `AssistantPlan`/`AssistantResult` metadata를 쓰게 한다. | M6 완료. `/api/ai/ask`는 wrapper-only facade로 기존 stream/job/artifact route에 위임하며, `NEXT_PUBLIC_AI_ASK_FACADE_ENABLED=true`일 때 streaming frontend path에서 opt-in 가능하다. |
...
| Provider reasoning/freshness policy | 무료 tier provider별 reasoning/thinking 지원과 smoke freshness를 policy에 기록한다. | reasoning 지원 여부는 provider별로 가능성이 있으나 현재 런타임 기본값과 계정 entitlement가 다르다. `lastVerified`, `expiresAt`, `smokeSource`는 아직 미구현. |
| MonitoringFactPack/eval guard | deterministic fact boundary와 retrieval/evidence quality eval을 명시한다. | M7 미구현. 같은 dataSlot/query scope의 사실 계산과 LLM 설명 책임을 타입/테스트로 분리해야 한다. |
...
| 리스크 | 영향 | 관찰 포인트 |
|--------|------|-------------|
| Frontend와 backend 라우팅 중복 | M5 planner shadow로 drift 측정은 가능해졌지만, 실제 authority는 아직 frontend/BFF/Cloud Run에 분산되어 있다. | M6에서 `/api/ask` facade를 wrapper-only로 도입하고, authority 이전은 drift 기준 통과 후로 제한한다. |
| API surface가 넓음 | `artifact-intent`, `incident-report`, `intelligent-monitoring`, `jobs`, `supervisor` 등 경로가 많아 contract drift 가능성이 있다. | `/api/ask`가 또 하나의 독립 route가 되면 복잡도가 늘어난다. 기존 route 내부 위임 facade로만 시작해야 한다. |
...
| Fact boundary 부재 | tool layer는 deterministic fact source처럼 동작하지만 `MonitoringFactPack` 타입이 없어 LLM 설명 책임과 metric severity 계산 책임이 섞일 수 있다. | M7에서 fact pack, evidenceCount=0 응답, retrieval suppressed reason eval을 묶어야 한다. |
...
| 보강 | 이유 |
|------|------|
| authoritative `AssistantPlan` 승격 | M6 wrapper-only facade까지 완료됐다. 다음 단계는 M7 `MonitoringFactPack`으로 fact boundary를 고정하고, drift/latency 기준 통과 후 routing authority를 점진 이전하는 것이다. |
...
| Provider smoke freshness | provider policy가 오래되면 런타임 장애로 이어진다. smoke timestamp, expiry, reasoning capability, account entitlement를 policy에 포함한다. |
| Trace id propagation to artifacts | 사용자가 보는 artifact id로 provider attempt와 tool result를 추적한다. |
| MonitoringFactPack | tool result가 이미 deterministic source처럼 동작하므로, metric severity/retrieval evidence를 하나의 fact boundary로 묶으면 LLM hallucination guard가 쉬워진다. |
...
2026-05-03 현재 코드 기준으로 실제 authority와 contract 위치를 다시 확인했다. 결론은 문서의 큰 방향과 일치한다. **M1~M6는 관측·정렬·wrapper 계층이고, 실제 authority 이전은 아직 아니다.**
...
| BFF stream proxy | `src/app/api/ai/supervisor/stream/v2/route.ts`가 `analysisMode`, `queryAsOf`, sanitized `localRouteDecision`을 Cloud Run으로 전달한다. | M6 `/api/ask`는 이 proxy 로직을 재구현하지 말고 감싸야 한다. 독립 구현이 되면 API surface가 늘어난다. |
...
| Planner shadow | `cloud-run/ai-engine/src/services/ai-sdk/supervisor-mode.ts`가 `resolveSupervisorModeDecision()`으로 실제 mode를 고르고, 별도로 `buildSupervisorPlannerShadowForRequest()`를 만들어 candidate/drift/latency를 plan metadata에 붙인다. | M5는 "실행 변경"이 아니라 "비교 측정"이다. M6/M7 전까지 shadow result를 제품 의사결정 근거로 축적해야 한다. |
| Monitoring fact boundary | `cloud-run/ai-engine/src/tools-ai-sdk/monitoring-tools.ts`와 `services/monitoring/monitoring-types.ts`는 `sourceMode`, `queryAsOf`, `evidenceRefs`를 이미 제공한다. | M7 `MonitoringFactPack`은 새 데이터를 만드는 작업보다 기존 tool result를 canonical bundle로 묶는 작업에 가깝다. |
...
| Provider policy | `cloud-run/ai-engine/src/services/ai-sdk/provider-model-policy.ts`는 deprecation/quota/capability/smokeEvidence를 갖지만 `lastVerified`, `expiresAt`, `reasoningCapability`는 없다. | provider-native thinking을 UI 계약으로 노출하기엔 아직 부족하다. 현재 `thinking` 버튼은 routing intensity로 유지하는 판단이 맞다. |

**개선 우선순위 재평가**:

1. M6 `/api/ask`는 기존 stream/job/artifact route를 **wrapper-only**로 감싸는 방식으로 완료됐다. 새 planner를 여기서 다시 만들면 A의 복잡도가 줄지 않는다.
2. M7 `MonitoringFactPack`은 `sourceMode/queryAsOf/evidenceRefs`를 하나의 canonical result로 묶어 E 원칙을 제품 contract로 고정하는 작업이다.
3. Artifact workspace/schema registry는 C 원칙을 강화하지만, M6/M7보다 먼저 하면 message metadata와 route authority drift 문제를 더 키울 수 있다.
...
8. 기존 route surface 위에 `/api/ask` facade를 wrapper-only로 추가하고, 새 기능은 facade-compatible request/response shape를 우선 사용한다. **완료: 2026-05-03 M6**
9. Provider policy smoke evidence에 `lastVerified`, `expiresAt`, `smokeSource`, `reasoningCapability`를 추가해 freshness 기준을 명시한다.
...
M1~M5c는 기존 라우팅을 바꾸지 않고 공통 metadata 언어와 shadow 측정 기반을 만든 단계다. 다음 gap은 facade endpoint, deterministic fact boundary, provider freshness, streaming UX를 실제 제품 계약으로 승격하기 전에 해소해야 한다.
...
| Streaming UI S1~S3 | stream status, latencyTier, thinking steps는 일부 표시된다. Cloud Run cold start indicator도 있다. | 전체 페이지 SSE, cold start countdown, agent step visibility가 M6 transport와 아직 정렬되지 않았다. | `/api/ask` transport와 같은 stream event contract로 progress, cold start, tool step, partial result를 표시 |
...
| 우선순위 | 질문 | 판단 기준 |
|:--:|------|-----------|
| 1 | `/api/ask` facade를 어느 범위까지 public entrypoint로 만들 것인가? | M6는 wrapper-only로 시작했고 기본 route는 유지한다. 기존 route를 다시 구현하거나 authority를 즉시 이전하면 route surface가 줄지 않고 늘어난다. |
