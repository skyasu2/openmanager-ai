...
| **Cerebras** | Orchestrator/Analyst/Reporter/Advisor/Verifier primary, text fallback | `qwen-3-235b-a22b-instruct-2507` → `llama3.1-8b` | Qwen primary. 8B는 context floor가 없는 짧은 경로의 intra-provider fallback으로만 허용. `gpt-oss-120b`는 현재 계정 chat completions 404로 runtime 후보에서 제외 |
...
Structured routing은 Orchestrator 정책(`Cerebras → Groq → Mistral`)을 따릅니다. 텍스트 에이전트는 quota 격리를 위해 Group A(Supervisor/NLQ)는 `Groq → Cerebras → Mistral`, Group B(Analyst/Reporter/Advisor/Verifier)는 `Cerebras → Groq → Mistral`을 사용합니다. Cerebras 내부에서는 Qwen을 먼저 시도하되, 16K/32K context floor가 걸린 경로에서는 `llama3.1-8b`를 건너뛰고 Groq로 넘어갑니다.
...
> SSOT: `agent-runtime-policy.ts`, `agent-model-selectors.ts`, `model-provider.ts` — Supervisor/NLQ는 Groq-first, Analyst/Reporter/Advisor/Verifier/Orchestrator는 Cerebras-first chain을 사용합니다.
