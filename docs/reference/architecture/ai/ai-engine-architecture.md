...
| single/multi/job/facade 판단은 어디서 하는가 | §2.6 Free Tier 소비, §6 Supervisor/Direct Router | [AI Agent 설계 경계](../../../design/README.md#ai-agent-설계-경계) |
...
현재 길이는 **1,500줄 초과**이며, Architecture Evaluation 섹션은 [ai-engine-evaluation.md](./ai-engine-evaluation.md)로 분리했습니다. core runtime 계약(Overview·Philosophy·System Architecture·Provider·Agent·Tool·Resilience)은 단일 독자·단일 참조 단위이므로 이 파일에서 유지합니다.
...
| **기반 모델** | Groq `llama-4-scout-17b-16e-instruct`, Z.AI `glm-4.5-flash` text, Mistral `mistral-small-latest`, Cerebras `gpt-oss-120b`, Gemini `gemini-2.5-flash-lite` Vision |
| **호스팅** | Groq, Z.AI, Mistral, Cerebras, Google AI (Gemini) 인프라 |
...
> **[ADR-005, 2026-05-16]** Orchestrator LLM routing과 `decomposeTask()` LLM decomposition은 기본 request path에서 제거됐습니다. 현재 multi-agent path는 Direct Router가 deterministic pre-filter fast path를 먼저 적용하고, confidence가 낮거나 agent hint가 없는 질의에만 Groq Scout structured classifier(`preFilterQueryWithLLM`)를 bounded fallback으로 사용한 뒤 specialist Tool-loop agent를 선택합니다.
>
...
flowchart LR
    ...
    Need -->|vision| Vision["Gemini Flash-Lite only"]
```
...
```
+--------------------------------------------------------------------+
| LLM PROVIDER SELECTION — Round-Robin + Context Guard               |
| cloud-run/ai-engine/src/services/ai-sdk/agents/config/             |
|   round-robin-provider-selector.ts                                 |
|   agent-model-selectors.ts                                         |
+--------------------------------------------------------------------+
...
Agent call
    ...
    v
+------------------------------------------+
| selectRoundRobinProviderOrder(minCtx)    |  <- module-level cursor 전진
+------------------------------------------+
    |
    +-- Eligible providers (context ≥ minCtx)  → rotation 선두
    `-- Ineligible providers (context < minCtx) → 후미 (제외 X, 순서만 후퇴)

Rotation pool (4개, 동등 가중치):
    cursor % 4 → [Groq, Mistral, Z.AI, Cerebras] 순환

CONTEXT_WINDOW (minContextTokens 기준):
    Groq    : 131,072 tokens
    Mistral : 32,000 tokens
    Z.AI    : 128,000 tokens
    Cerebras: 65,536 tokens (gpt-oss-120b)

Per-agent minContextWindow 요구:
    Supervisor          : 8K
    Metrics Query       : 16K
    Analyst/Reporter/Advisor/Verifier : 32K
    → production gpt-oss-120b 기준 4개 provider 모두 eligible
...
Vision path (별도 selector — round-robin 미적용):
    |
    +-- Gemini
          model: GEMINI_VISION_MODEL_ID || gemini-2.5-flash-lite

Vercel artifact intent classifier (별도 selector — round-robin 미적용):
    |
    `-- Mistral ministral-3b-latest
...
Supervisor는 실행 모드(`single`/`multi`)를 정하고, multi로 resolve된 요청만 Direct Router로 넘깁니다. Direct Router는 deterministic pre-filter fast path와 제한된 LLM intent classifier fallback으로 specialist agent를 선택합니다. 기본 request path에서 Orchestrator LLM routing과 `decomposeTask()` LLM decomposition은 호출하지 않습니다.
...
| **방법** | complexity + explicit mode gate + tool filtering | deterministic pre-filter fast path + bounded LLM classifier fallback + direct specialist routing |
...
## 3. Provider 선택 전략 — Round-Robin + Context Guard

텍스트 에이전트는 2026-05-16부터 **Round-Robin + Context Guard** 방식으로 provider를 선택합니다. 에이전트별 고정 1차 provider는 제거되고, 모듈 레벨 커서가 Groq → Mistral → Z.AI → Cerebras를 균등하게 순환합니다. 요청 에이전트가 요구하는 최소 context window(minContextTokens)를 충족하지 못하는 provider는 순서 후미로 밀리지만 제외되지는 않습니다.

**장점**: 무료 tier RPD/RPM 쿼터를 4개 provider에 자동 분산, 에이전트별 수동 provider 배치 유지보수 제거, `rotationSlot`을 통한 사용자 가시적 provider attribution 지원.
**현재 gap**: 실시간 429 카운터 기반 cooldown 미구현 (circuit breaker는 이미 있음). Z.AI 실제 RPM 한도 미검증(보수적 5 RPM 가정).

| Provider | 모델 | Context | 무료 한도 (참고치) | 운영 메모 |
|----------|------|---------|-------------------|-----------|
| **Groq** | `meta-llama/llama-4-scout-17b-16e-instruct` | 131K | 30 RPM / 1K RPD / 500K TPD | tool-calling 지원. 모든 에이전트 eligible |
| **Mistral** | `mistral-small-latest` | 32K | 50 RPM / 50K TPM (workspace 의존) | 32K border에서 Analyst/Reporter/Advisor는 eligible (경계값 포함). runtime guard는 보수적 유지 |
| **Z.AI** | `glm-4.5-flash` | 128K | ~5 RPM (미검증) | Flash text fallback 전용. Web Search는 유료라 runtime 금지. AI SDK OpenAI-compatible path + `thinking: disabled` 필요. Text agents eligible |
| **Cerebras** | `gpt-oss-120b` | 65K | 5 RPM / 2.4K RPD / 1M TPD | 2026-05-16 gpt-oss-120b 전환 완료. 65K context로 모든 에이전트 eligible. tool-calling은 `CEREBRAS_TOOL_CALLING_ENABLED=false` 기본값 유지 |
| **Gemini** | `gemini-2.5-flash-lite` | — | — | Vision primary 전용. Round-Robin 미적용 |
...
| Cerebras | `gpt-oss-120b` 등 reasoning 모델 지원 | 2026-05-16 `gpt-oss-120b` 전환 완료. 65K context 제공. reasoning capability는 별도 smoke 후 opt-in 필요 | opt-in 후보 (tool-calling은 `CEREBRAS_TOOL_CALLING_ENABLED=false` 기본 유지) |
...
### Provider 선택 순서 (Round-Robin 기반, 호출 시점마다 달라짐)

2026-05-16 이후 텍스트 에이전트는 고정 primary가 아닌 Round-Robin 커서 위치에 따라 첫 번째 provider가 결정됩니다. 아래 표는 minContextWindow 요구사항 기준 eligible/ineligible 분류입니다.

| Agent | minContext | 커서 위치 예시 (변동) | Ineligible (후미 이동) |
|-------|-----------|----------------------|------------------------|
| Supervisor | 8K | 순환 중 현재 커서 위치 | 없음 (4개 모두 eligible) |
| Metrics Query | 16K | 순환 중 현재 커서 위치 | 없음 (4개 모두 eligible) |
| Analyst | 32K | 순환 중 현재 커서 위치 | Mistral(32K, 경계값 포함) |
| Reporter | 32K | 순환 중 현재 커서 위치 | Mistral(32K, 경계값 포함) |
| Advisor | 32K | 순환 중 현재 커서 위치 | Mistral(32K, 경계값 포함) |
| Vision | — | 고정 (round-robin 미적용) | — |

> SSOT: `round-robin-provider-selector.ts` (커서/rotation pool), `agent-model-selectors.ts` (per-agent minContextWindow). 상세 provider 한도/모델 ID 환경변수 → [`free-tier-optimization.md`](../infrastructure/free-tier-optimization.md#part-5-llm-프로바이더-비용-제어)
...
│   │   ├── model-provider-core.ts     # Provider 생성 (Groq/Z.AI/Mistral/Cerebras/Gemini)
...
| 도구 | 30개 (8개 카테고리) |
| LLM Provider | 5개 (Groq, Z.AI, Mistral, Cerebras, Gemini) |
| Fallback 체인 | Round-Robin + Context Guard text mesh + Vision Gemini-only degradation |
...
- **Vision 정합성 수정**: 당시 `gemini-2.5-flash-lite` 기본화와 Z.AI Vision fallback을 정리했다. 2026-05-20 이후 current runtime은 Gemini-only다.
...
- **Vision Agent**: 당시 Gemini 2.5 Flash + Z.AI Vision fallback. 2026-05-20 이후 current runtime은 Gemini-only다.
...
- **[Architecture Evaluation](./ai-engine-evaluation.md)** - 성숙도 평가, 성능 기준선, 미결 개선 항목
