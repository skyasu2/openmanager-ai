...
OpenManager AI의 AI Engine은 **Vercel AI SDK v6 계열** 기반 **deterministic/single-first + conditional multi-agent escalation** 아키텍처입니다.
...
즉, 현재 시스템은 "멀티에이전트 전용"이 아니라 **도메인 fact와 저비용 single path를 기본값으로 두고, 필요한 경우에만 multi-agent로 승격하는** 구조입니다.
...
> **As-built note (2026-05-16)**: 이 문서는 초기 설계도가 아니라 실제 구현을 역추적한 현재 아키텍처 기준입니다. 기본 채팅 transport는 `/api/ai/supervisor/stream/v2`입니다. 최근 안정화는 Z.AI 무료 GLM Flash 기반 provider mesh, Cerebras `llama3.1-8b` deprecation contingency, intent별 LLM parameter, tool-result response enrichment, deterministic ranking/recovery fallback을 중심으로 이루어졌습니다.
...
flowchart TB
    ...
    subgraph CR["Cloud Run AI Engine (Hono)"]
        ...
        Route["Structured Output Fallback\nGroq → Z.AI → Mistral → Cerebras\n(generateText + Output.object)"]
...
LLM path
  ...
  multi  -> preFilterQuery()
            ...
            +-- structured route -> Agent execution
            `-- forced/fallback route -> Agent execution
...
│  │  ├─ Structured routing (generateText + Output.object)  │ │
...
Groq-first: Supervisor / Orchestrator / Metrics Query
    |
    +-- Groq
...
Cerebras-first: Analyst / Verifier
    |
    +-- Cerebras
    |     model: llama3.1-8b
...
| **실행** | `streamText()` 중심 실시간 스트리밍 | `generateText + Output.object` structured routing, `generateTextWithRetry`, `streamText` |
...
|----------|----------------|------|-----------|
| **Groq** | Supervisor/Metrics Query/Orchestrator primary, fallback for long-context agent paths | `meta-llama/llama-4-scout-17b-16e-instruct` | tool-calling 중심 텍스트 primary. 공식 Free Plan 기준 30 RPM / 1K RPD / 30K TPM / 500K TPD |
...
| **Cerebras** | Short-context text fallback until cutoff | `llama3.1-8b` | 현재 계정 chat completion smoke는 HTTP 200. 단 8K context이며 2026-05-27 deprecation 예정. 2026-05-16 header 기준 5 RPM / 30K TPM / 2.4K RPD / 1M TPD |
...
| Mistral | `mistral-small-latest`가 `reasoning_effort=high|none` adjustable reasoning 지원 | Cloud Run `@ai-sdk/mistral@3.0.20`에는 `reasoningEffort` provider option이 없고, last-resort text fallback로만 사용 | 패키지 업데이트 또는 직접 REST smoke 후 후보 |
...
텍스트 에이전트는 quota 격리를 위해 provider 순서를 역할별로 회전합니다. Cerebras runtime은 `llama3.1-8b`만 기본 후보로 두며, 16K/32K context floor가 걸린 경로에서는 capability gate가 Cerebras를 건너뛰고 Groq/Z.AI/Mistral로 넘어갑니다. 2026-05-27 이후 replacement entitlement가 없으면 Cerebras-first 경로는 `Groq → Z.AI → Mistral`로 사전 전환됩니다.

| Agent | Primary | → 2nd | → 3rd | → 4th |
|-------|---------|-------|------|------|
| Supervisor | Groq | Z.AI | Mistral | Cerebras |
| Orchestrator | Groq | Z.AI | Mistral | Cerebras |
| Metrics Query | Groq | Z.AI | Mistral | Cerebras |
| Analyst | Cerebras | Groq | Z.AI | Mistral |
| Reporter | Z.AI | Mistral | Groq | Cerebras |
| Advisor | Mistral | Z.AI | Groq | Cerebras |
| Verifier | Cerebras | Groq | Z.AI | Mistral |
...
Multi-agent stream done metadata에는 non-stream과 같은 sanitized `routingDecisionTrace`가 포함됩니다. pre-filter, LLM routing, fallback source는 `agentDecision.source`로 구분합니다.
...
Vercel AI SDK v6 공식 agent 문서 기준으로 현재 구현을 재평가한 결과, OpenManager는 **AI SDK primitive 기반 custom orchestrator** 구조에 해당한다. 즉, `ToolLoopAgent`/`streamText`/`generateText + Output.object`/`UIMessageStream`은 적극 사용하지만, 모든 handoff를 AI SDK subagent-as-tool 패턴으로 모델링하지는 않는다.
...
| Loop control | single path는 `prepareStep`으로 activeTools/toolChoice를 제어. multi forced/stream path는 role별 tool allowlist + `stopWhen` 중심 | 부분 부합 |
| Structured routing | Orchestrator routing/decomposition은 `generateText + Output.object` + Zod + text JSON fallback | 강하게 부합 |
| Subagents | custom Orchestrator가 specialist 함수를 호출하고 병렬 subtasks를 `Promise.all`로 실행 | 기능은 있음. AI SDK subagent-as-tool idiom은 미적용 |
...
| **Supervisor→Orchestrator→Agent 계층** | 잘 설계됨 | Single-Agent(`streamText`)와 Multi-Agent(`generateText + Output.object` structured routing + agent tool loop)의 분리가 명확. `executeForcedRouting`이 BaseAgent 우회하는 이중 경로는 여전히 존재 |
...
| 항목 | 상세 분석 |
|------|-----------|
| **모델 배분** | Groq는 Supervisor/Metrics Query/Orchestrator primary, Z.AI는 Reporter primary 및 free Flash fallback, Mistral은 Advisor primary 및 distributed fallback, Cerebras는 2026-05-27 전까지 short-context fallback, Gemini는 Vision primary, OpenRouter/Z.AI Vision은 Vision fallback으로 역할이 분리됨 |
...
| Analyst | Cerebras `llama3.1-8b` when context permits, otherwise Groq → Z.AI → Mistral | tool-calling text path | 8 | — |
