...
| **OpenRouter** | `gemma-3-27b-it:free` 등 | — | credit 의존 | Vision fallback 전용. Round-Robin 미적용 |
...
| 항목 | 상세 분석 |
|------|-----------|
| **모델 배분** | Text agent는 고정 primary 대신 `Groq → Mistral → Z.AI → Cerebras` round-robin cursor와 context/capability gate로 선택한다. Vision은 Gemini primary, OpenRouter/Z.AI Vision fallback 고정 경로를 유지한다 |
...
| Vision | Gemini `gemini-2.5-flash-lite` → OpenRouter → Z.AI Vision | multimodal primary + fallback | 2 | — |
