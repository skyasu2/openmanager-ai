...
텍스트 에이전트는 quota 격리를 위해 Group A(Supervisor/NLQ/Orchestrator)는 `Groq → Cerebras → Mistral`, Group B(Analyst/Reporter/Advisor/Verifier)는 `Cerebras → Groq → Mistral`을 사용합니다. Cerebras 내부에서는 Qwen을 먼저 시도하되, 16K/32K context floor가 걸린 경로에서는 `llama3.1-8b`를 건너뛰고 Groq로 넘어갑니다.
