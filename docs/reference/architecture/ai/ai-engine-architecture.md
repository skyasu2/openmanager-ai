...
| **기반 모델** | Groq `llama-4-scout-17b-16e-instruct`, Z.AI `glm-4.5-flash`/`glm-4.6v-flash`, Mistral `mistral-small-latest`, Cerebras `gpt-oss-120b`, Gemini `gemini-2.5-flash-lite` |
...
Cloud Run AI Engine
  ...
  |     `-- vision tools -> Gemini / Z.AI Vision
  |
  |-- short-lived context
...
flowchart LR
    ...
    Need -->|vision| Vision["Gemini Flash-Lite → Z.AI Vision"]
```
...
Vision attachments
    |
    `-- Vision Agent (Gemini -> Z.AI Vision)
...
| **Z.AI** | `glm-4.5-flash`, `glm-4.6v-flash` | 128K | ~5 RPM (미검증) | Flash text/vision 무료. Web Search 유료라 runtime 금지. AI SDK OpenAI-compatible path + `thinking: disabled` 필요. 모든 에이전트 eligible |
...
| Fallback 체인 | Round-Robin + Context Guard text mesh + Vision Gemini/Z.AI fallback |
...
- **Vision 정합성 수정**: `gemini-2.5-flash-lite` 기본화, Z.AI Vision fallback 정리
...
- **Vision Agent**: Gemini 2.5 Flash + Z.AI Vision Fallback
...
| 항목 | 상세 분석 |
|------|-----------|
| **모델 배분** | Text agent는 고정 primary 대신 `Groq → Mistral → Z.AI → Cerebras` round-robin cursor와 context/capability gate로 선택한다. Vision은 Gemini primary, Z.AI Vision fallback 고정 경로를 유지한다 |
...
| 항목 | 평가 | 상세 |
|------|------|------|
| **Round-Robin Provider Mesh** | 적절 | provider capability gate + circuit breaker + quota tracker + retry/fallback 순서가 명확하고, 429 cooldown provider를 후순위로 밀어 무료 tier provider에 부하를 분산함 |
...
| Vision | Gemini `gemini-2.5-flash-lite` → Z.AI Vision | multimodal primary + fallback | 2 | — |
| Evaluator | 결정론적 (LLM 없음) | pipeline internal | 3 | — |
| Optimizer | 결정론적 (LLM 없음) | pipeline internal | 3 | — |

> Latency는 provider 정책, quota, attachment size, routing path에 따라 크게 변동하므로 이 문서에서는 절대 수치 대신 **현재 provider selection 계약**만 SSOT로 유지합니다.
