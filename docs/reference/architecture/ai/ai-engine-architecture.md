...
>
> **v8.10.8** | Updated 2026-03-30
> (ai-model-policy.md 내용 통합됨, 2026-02-14)

## 1. Overview

OpenManager AI의 AI Engine은 **Vercel AI SDK v6** 기반 **Multi-Agent System**입니다.
프론트엔드는 `useChat` + 커스텀 `DefaultChatTransport`를 사용하고, reconnect 요청 경로는 준비되어 있으나 `resume` 플래그는 현재 기본 비활성입니다. Cloud Run은 `streamText`, `generateText`, `generateObject`, BaseAgent 내부 `ToolLoopAgent`, 서버 측 resumable-compatible wrapper를 조합한 하이브리드 실행 구조를 사용합니다.
...
| **기반 모델** | Cerebras text model (`CEREBRAS_MODEL_ID`, default `qwen-3-235b-a22b-instruct-2507`), Groq llama-3.3-70b, Mistral Large, Gemini 2.5 Flash |
...
│  ├────────────────────────────────────────────────────────┤ │
│  │ 5 Work Agents + 2 Pipeline-Internal Quality Stages × 5 Providers │ │
│  └────────────────────────────────────────────────────────┘ │
│           │              │              │                     │
│     ┌─────┴─────┐  ┌────┴────┐  ┌─────┴──────┐             │
...
> Source of truth (2026-03-30): `cloud-run/ai-engine/src/services/ai-sdk/agents/config/agent-configs.ts` (config 7개: 작업 5 + 내부 품질 단계 2), `cloud-run/ai-engine/src/server.ts` `app.route('/api/...')` (API mounts 9), `cloud-run/ai-engine/src/routes/*.ts` (route modules 10).
...
|----------|----------------|------|-----------|
| **Cerebras** | Supervisor, NLQ, Analyst, Orchestrator | `CEREBRAS_MODEL_ID` (current default: `qwen-3-235b-a22b-instruct-2507`) | 핵심 질의 경로 담당. 계정별 접근 모델과 무료/저비용 정책은 공급사 정책 변동 가능 |
| **Groq** | Reporter | `llama-3.3-70b-versatile` (70B) | 빠른 보고서 생성 경로. 분당 요청 제한이 첫 병목이 되기 쉬움 |
| **Mistral** | Advisor + RAG Embedding | `mistral-large-latest` / `mistral-embed` (1024d) | Advisor와 임베딩을 함께 담당. Tier 정책은 수시 변경 가능 |
| **Gemini** | Vision | `gemini-2.5-flash` (1M context) | Vision 전용 primary. 무료 플랜은 burst 요청에 취약 |
| **OpenRouter** | Vision Fallback | `google/gemma-3-4b-it:free` (fallback: `nvidia/nemotron-nano-12b-v2-vl:free`, `mistralai/mistral-small-3.1-24b-instruct:free`) | Vision fallback 전용. 가용 모델은 provider 상태에 영향받음 |
...
| AI SDK | `ai` 6.0.138 / `@ai-sdk/react` 3.0.140 (현재 프로젝트 적용 버전) |
...
| **ToolLoopAgent (BaseAgent)** | 작업 에이전트 내부 루프 패턴 |
| **generateObject** | 구조화 라우팅 및 fallback 파싱 (Orchestrator) |
...
## 13. Architecture Evaluation (2026-03-30)
...
| **Cerebras text-model 활용성** | 강함 | Supervisor/NLQ/Analyst/Orchestrator의 핵심 텍스트 추론 경로를 Cerebras primary로 집중 배치. 기본 모델은 환경변수로 조정 가능 |
| **AI SDK v6 구현 성숙도** | 높음 | BaseAgent 계층은 ToolLoopAgent를 사용하고, Supervisor/Orchestrator 일부 경로는 `generateText`/`generateObject`를 직접 사용함. 즉 SDK v6의 여러 실행 패턴을 혼합 적용 |
...
| 항목 | 상세 분석 |
|------|-----------|
| **모델 배분** | Cerebras는 텍스트 중심 핵심 경로, Groq는 Reporter, Mistral은 Advisor/Embedding, Gemini는 Vision primary, OpenRouter는 Vision fallback으로 역할이 분리됨 |
| **AI SDK 적용 방식** | 프론트엔드는 `useChat`/`DefaultChatTransport`, 서버는 `streamText`/`generateText`/`generateObject`, 작업 에이전트는 BaseAgent 내부 `ToolLoopAgent`를 사용함 |
| **문서 검증 범위** | 2026-03-30 기준 코드로 직접 검증한 항목은 버전, 에이전트 구성, 라우팅 임계값, pipeline 결정론, transport/resumable 구조임. 외부 provider 무료 티어/성능 수치는 별도 재검증 필요 |
