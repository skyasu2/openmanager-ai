...
| **기반 모델** | Groq `llama-4-scout-17b-16e-instruct`, Cerebras `llama3.1-8b`, Mistral `mistral-small-latest`, Gemini `gemini-2.5-flash-lite` |
...
Group A: Supervisor / Orchestrator / Metrics Query
    |
    +-- Groq
...
|----------|----------------|------|-----------|
| **Groq** | Supervisor/Metrics Query primary, Cerebras-first fallback | `meta-llama/llama-4-scout-17b-16e-instruct` | tool-calling 중심 텍스트 fallback 및 단순 경로 primary |
| **Cerebras** | Short-context text fallback | `llama3.1-8b` | 2026-05-13 현재 계정 chat completion smoke는 HTTP 200. 단 8K context이며 2026-05-27 deprecation 예정이므로 Metrics Query 16K primary 승격 근거로 쓰지 않음. Qwen 235B는 429 queue/quota, `gpt-oss-120b`와 `zai-glm-4.7`은 current key chat completions 404로 runtime 후보 제외 |
| **Mistral** | Text last-resort fallback | `mistral-small-latest` | 무료 티어 보호를 위해 Small 계열을 기본값으로 사용. 내부 지식 검색 runtime/embedding에는 사용하지 않음 |
...
텍스트 에이전트는 quota 격리를 위해 Group A(Supervisor/Metrics Query/Orchestrator)는 `Groq → Cerebras → Mistral`, Group B(Analyst/Reporter/Advisor/Verifier)는 `Cerebras → Groq → Mistral`을 사용합니다. Cerebras runtime은 `llama3.1-8b`만 기본 후보로 두며, 16K/32K context floor가 걸린 경로에서는 capability gate가 Cerebras를 건너뛰고 Groq로 넘어갑니다. 2026-05-27 이후 replacement entitlement가 없으면 Group B의 실효 fallback은 `Groq → Mistral`로 축소되고, Mistral 2 RPM은 burst 흡수용으로 충분하지 않습니다.

| Agent | Primary | → 2nd | → 3rd (Last Resort) |
|-------|---------|-------|---------------------|
| Supervisor | Groq | Cerebras | Mistral |
| Orchestrator | Groq | Cerebras | Mistral |
| Metrics Query | Groq | Cerebras | Mistral |
| Analyst | Cerebras | Groq | Mistral |
| Reporter | Cerebras | Groq | Mistral |
| Advisor | Cerebras | Groq | Mistral |
| Verifier | Cerebras | Groq | Mistral |
| Vision | Gemini | OpenRouter | — |

> SSOT: `agent-runtime-policy.ts` → Orchestrator는 v8.11.65부터 Groq-first. Cerebras account quota는 2026-04-30 기준 30 RPM / 14.4K RPD이며, `llama3.1-8b` deprecation contingency는 `provider-model-policy.ts`와 `memory/ops-knowledge.md`에 기록합니다. 상세 provider 한도/모델 ID 환경변수 → [`free-tier-optimization.md`](../infrastructure/free-tier-optimization.md#part-5-llm-프로바이더-비용-제어)
...
| 항목 | 상세 분석 |
|------|-----------|
| **모델 배분** | Groq는 Supervisor/Metrics Query/Orchestrator primary, Cerebras는 Analyst/Reporter/Advisor policy의 primary이되 context/capability gate에 따라 skip 가능, Mistral은 `mistral-small-latest` last-resort text fallback, Gemini는 Vision primary, OpenRouter는 Vision fallback으로 역할이 분리됨 |
...
|-------|-----------------|------------|:----------:|:------------:|
| Metrics Query | Groq `llama-4-scout-17b-16e-instruct` | tool-calling text path | 10 | — |
| Analyst | Cerebras `llama3.1-8b` when context permits, otherwise Groq | tool-calling text path | 8 | — |
| Reporter | Cerebras `llama3.1-8b` when context permits, otherwise Groq | Reporter pipeline + tool path | 12 | score ≥ 0.75 |
...
| Orchestrator | Groq primary, Cerebras `llama3.1-8b` fallback | structured output routing | — | — |
