...
> **As-built note (2026-05-04)**: 이 문서는 초기 설계도가 아니라 실제 구현을 역추적한 현재 아키텍처 기준입니다. 기본 채팅 transport는 `/api/ai/supervisor/stream/v2`이고, `/api/ai/ask`는 기존 stream/job/artifact route를 재구현하지 않는 wrapper-only opt-in facade입니다. 최근 안정화는 planner shadow metadata, formatting-only rewrite guard, deterministic ranking/recovery fallback을 중심으로 이루어졌습니다.
...
Result handling in useAIChatCore.ts
    ...
    `-- none                 -> sendQuery() to /api/ai/supervisor/stream/v2
                                or /api/ai/ask when facade flag is enabled
...
|----------|----------------|------|-----------|
| **Groq** | Supervisor/NLQ primary, Cerebras-first fallback | `meta-llama/llama-4-scout-17b-16e-instruct` | tool-calling 중심 텍스트 fallback 및 단순 경로 primary |
| **Cerebras** | Short-context text fallback | `llama3.1-8b` | 현재 계정에서 production smoke가 통과한 Cerebras runtime. Qwen 235B는 Preview/high-traffic으로 기본 후보 제외. `gpt-oss-120b`는 공식 production 모델이지만 현재 계정 chat completions 404로 runtime 후보 제외 |
...
텍스트 에이전트는 quota 격리를 위해 Group A(Supervisor/NLQ/Orchestrator)는 `Groq → Cerebras → Mistral`, Group B(Analyst/Reporter/Advisor/Verifier)는 `Cerebras → Groq → Mistral`을 사용합니다. Cerebras runtime은 `llama3.1-8b`만 기본 후보로 두며, 16K/32K context floor가 걸린 경로에서는 capability gate가 Cerebras를 건너뛰고 Groq로 넘어갑니다.
...
> SSOT: `agent-runtime-policy.ts` → Orchestrator는 v8.11.65부터 Groq-first (Cerebras RPM 5/min 절약). 상세 provider 한도/모델 ID 환경변수 → [`free-tier-optimization.md`](../infrastructure/free-tier-optimization.md#part-5-llm-프로바이더-비용-제어)
