...
텍스트 에이전트는 quota 격리를 위해 provider 순서를 역할별로 회전합니다. Cerebras runtime은 `llama3.1-8b`만 기본 후보로 두지만, 8K context와 2026-05-27 deprecation 때문에 16K/32K 장문 에이전트 primary에서는 제외하고 short-context last fallback으로만 유지합니다.
