...
이 문서는 AI Assistant request path에서 prompt injection과 off-domain 입력을 어떻게 처리하는지 정리합니다. 정책의 목적은 `high` 위험 입력은 차단하고, 애매하거나 범위 밖인 질문은 사용자에게 경고를 보여준 뒤 LLM이 한계를 설명하며 답하도록 위임하는 것입니다.
...
| Off-domain warning | `cloud-run/ai-engine/src/lib/off-domain-guard.ts` |
| Stream warning prepend | `cloud-run/ai-engine/src/services/ai-sdk/supervisor-stream.ts` |
| Non-stream warning prepend | `cloud-run/ai-engine/src/services/ai-sdk/supervisor-single-agent.ts` |
...
flowchart TB
    ...
    OffDomain -->|yes| OffDomainWarn["queue off-domain warning\nshouldShortCircuit=false"]
...
| Off-domain, 운영 컨텍스트 없음 | 계속 진행 | off-domain 경고 1줄 prepend |
...
경고 문구는 metadata-only 계약이 아니라 assistant 응답 본문 또는 SSE `text_delta` 앞에 삽입됩니다. 현재 frontend는 별도 warning banner를 요구하지 않습니다.

## 보안/비용 균형

- `high` risk는 무료 tier나 UX보다 안전을 우선해 차단합니다.
- `medium`/`low`는 false positive 가능성이 있으므로 sanitize + warning으로 처리하고, 모델 호출은 계속 허용합니다.
- off-domain은 제품 범위를 벗어났다는 신호를 사용자에게 명확히 주되, 단순 차단하지 않습니다.
- deterministic refusal, domain evidence, service command는 LLM 호출 없이 종료해 비용과 지연을 줄입니다.
