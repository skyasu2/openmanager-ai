...
| 기능 | Frontend | Backend | 평가 |
|------|:--------:|:-------:|:----:|
| Hybrid Query Router | 복잡도 기반 자동 분기 (523줄, 2026-05-19 측정) | stream/json 듀얼 모드 | 양쪽 완벽 |
...
### Frontend 핵심 파일 (대표 6개 총 2,938줄, 2026-05-19 재측정)

| 파일 | 역할 | 줄 수 | 비중 |
|------|------|:-----:|:----:|
| `src/stores/useAISidebarStore.ts` | Zustand 상태관리 | 674 | 23% |
| `src/hooks/ai/useAIChatCore.ts` | 공유 채팅 훅 | 578 | 20% |
| `src/hooks/ai/useHybridAIQuery.ts` | Hybrid Query Router | 523 | 18% |
...
```
AISidebarV4 (451줄)
├── useAIChatCore (578줄) ──→ useHybridAIQuery (523줄)
...
| # | 영역 | 현재 상태 | 개선 방향 | 우선순위 |
|---|------|----------|----------|:--------:|
| F2-r | useHybridAIQuery 추가 분할 | 523줄 (2026-05-19 재측정) | 1차 목표 완료. ≤500은 2차 후속 후보 | Low |
...
**양쪽 완성도가 거의 동등합니다.** 초기 분석 이후 Rate Limiting, CB Orchestrator 통합, AI 에러 컨텍스트 로그, 대용량 파일 분할, planner shadow metadata, deterministic recovery/formatting guard가 누적되어 종합 94% 수준을 유지합니다. 잔여 과제는 AI hooks 진입 맵과 route catalog 정리입니다.

### 코드 규모 비교

| 측면 | Frontend | Backend |
|------|:--------:|:-------:|
| 핵심 파일 | 대표 6개 (2,938줄, 2026-05-19 재측정) | 108개 TypeScript 파일 |
| 가장 큰 파일 | useAISidebarStore.ts (674줄) | model-provider.ts (680줄) |
| 도구/유틸리티 | 보안+캐시+에러 핸들링 | 모듈형 AI 도구 세트 |
| 아키텍처 분할 | 단일 파일 중심 | Modular Split (Supervisor 5, Orchestrator 6) |

### 조치 권장 사항 (Low 우선순위)

1. **AI hooks 진입 맵 작성**: `ai-assistant-improvement-plan.md` D4 — 9개 hook entry 매트릭스
2. **useHybridAIQuery 2차 분할 후보**: 현재 523줄 기준. ≤500은 후속 polish로 검토
