...
|------|:--------:|:-------:|:----:|
| 메시지 송수신 | useAIChatCore (607줄, 2026-05-19 측정) | Supervisor Stream V2 + legacy JSON/text route | 양쪽 완벽 |
...
| 기능 | Frontend | Backend | 평가 |
|------|:--------:|:-------:|:----:|
| Hybrid Query Router | 복잡도 기반 자동 분기 (704줄, 2026-05-19 측정) | stream/json 듀얼 모드 | 양쪽 완벽 |
...
| 아티팩트 인텐트 분류기 | `src/lib/ai/chat-artifacts/chat-artifact-intent.ts` | 3-tier artifact gate (local regex → LLM candidate gate → Vercel route): `incident-report\|monitoring-analysis\|server-snapshot\|guidance\|none`, 부정 패턴은 실행/implicit/LLM 후보 경로를 차단하고 guidance priority는 action보다 먼저 평가. `server-snapshot`은 regex-only/client-only이며 broad 서버 상태 질문은 일반 채팅 유지. 112-case eval guard: execution precision ≥ 0.94, all-class precision/recall ≥ 0.90 |
...
### Frontend 핵심 파일 (총 3,053줄, 2026-05-19 재측정)

| 파일 | 역할 | 줄 수 | 비중 |
|------|------|:-----:|:----:|
| `src/hooks/ai/useHybridAIQuery.ts` | Hybrid Query Router | 704 | 23% |
| `src/stores/useAISidebarStore.ts` | Zustand 상태관리 | 674 | 22% |
| `src/hooks/ai/useAIChatCore.ts` | 공유 채팅 훅 | 607 | 20% |
| `src/components/ai-sidebar/AISidebarV4.tsx` | 메인 사이드바 UI | 451 | 15% |
...
> **LOC drift 정리 (2026-05-19)**: 이전 측정(useAIChatCore 426 / useHybridAIQuery 909 / useAISidebarStore 551 / security 596)은 stale 상태였습니다. 현 plan: [ai-assistant-improvement-plan.md](../../../../reports/planning/ai-assistant-improvement-plan.md) — useAIChatCore ≤580, useHybridAIQuery ≤600 1차 목표.
...
```
AISidebarV4 (451줄)
├── useAIChatCore (607줄) ──→ useHybridAIQuery (704줄)
...
├── useAISidebarStore (674줄)
├── useResizable
└── useUserPermissions

route.ts (414줄) ──→ security.ts (298줄) + security-patterns.ts (254줄)
├── cloud-run-handler        ├── Injection 패턴 모듈 분리
...
└── Conditional Multi-Agent escalation (복잡 쿼리)
    └── Orchestrator
        ...
        ├── Direct Routing (Orchestrator LLM routing 제거)
...
1. **Hybrid Query Router (704줄, 2026-05-19 측정)**: 복잡도 점수 기반 자동 분기 (simple/moderate/complex/very_complex)
...
| # | 영역 | 현재 상태 | 개선 방향 | 우선순위 |
|---|------|----------|----------|:--------:|
| F2-r | useHybridAIQuery 추가 분할 | 704줄 (2026-05-19 재측정) | 쿼리 분류/metadata merge helper 추가 추출로 ≤600 1차, ≤500 2차 후속 | Low |
...
| 측면 | Frontend | Backend |
|------|:--------:|:-------:|
| 핵심 파일 | 6개 (3,053줄, 2026-05-19 재측정) | 108개 TypeScript 파일 |
| 가장 큰 파일 | useHybridAIQuery.ts (704줄) | model-provider.ts (680줄) |
| 도구/유틸리티 | 보안+캐시+에러 핸들링 | 모듈형 AI 도구 세트 |
| 아키텍처 분할 | 단일 파일 중심 | Modular Split (Supervisor 5, Orchestrator 6) |

### 조치 권장 사항 (Low 우선순위)

1. **useHybridAIQuery 추가 분할**: 현재 704줄 기준. ai-assistant-improvement-plan.md C3 — ≤600 목표(1차), ≤500 목표(2차)
2. **useAIChatCore hybrid 분기 헬퍼 추출**: 607줄 → ≤580 (`ai-assistant-improvement-plan.md` C2)
3. **route catalog 정리**: API route 목록과 테스트 카탈로그를 최신 구조에 맞게 유지
...
_Last Updated: 2026-05-19 (LOC drift and routing alignment)_
