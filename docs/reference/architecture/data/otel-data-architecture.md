...
**AI 데이터 소스 계층**: 2026-04-30부터 AI 어시스턴트의 Chat/Reporter/Analyst는 Cloud Run `MonitoringDataSource` 계약을 통해 같은 snapshot/evidence refs를 소비합니다. 기본 provider는 `replay-json`이며, 실제 서버 연결은 같은 interface의 `live-otel` provider skeleton으로 확장합니다. Vercel은 전체 분석 fan-out을 수행하지 않고 `/api/ai/monitoring/analyze-batch`로 1회 proxy합니다.
...
| `live-otel` | disabled skeleton | `MONITORING_SOURCE_MODE=live-otel` 설정 시 선택 가능하나 `LIVE_OTEL_ENDPOINT`가 없으면 `LIVE_SOURCE_DISABLED`로 명시 실패. 실제 OTLP/Prometheus/Loki adapter 구현은 별도 작업 |

공통 응답에는 `sourceMode`, `queryAsOf`, `slot`, `riskSignals`, `evidenceRefs`, `dataFreshness`를 포함합니다. Reporter와 Analyst는 이 계약을 기준으로 같은 시간 슬롯과 같은 증거를 사용합니다.
