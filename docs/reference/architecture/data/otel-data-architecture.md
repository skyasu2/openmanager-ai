...
공통 성공 응답에는 `sourceMode`, `queryAsOf`, `slot`, `riskSignals`, `evidenceRefs`, `dataFreshness`를 포함합니다. Monitoring source 오류 응답에는 `code`, `sourceMode`, `queryAsOf`, `requestId`, `recoverable`을 포함하며, Vercel BFF는 이를 fallback으로 숨기지 않고 pass-through합니다. Reporter와 Analyst는 이 계약을 기준으로 같은 시간 슬롯과 같은 증거를 사용합니다.
