...
```
 ...
 replay-json/live-otel           │
          │                       │
          ▼                       │
 Cloud Run AI endpoints/tools ◀──┘
 Chat / Reporter / Analyst snapshot grounding
```
...
### 5.1 MonitoringDataSource Provider 전환

| Provider | 상태 | 동작 |
|---|---|---|
| `replay-json` | 기본 활성 | Cloud Run 이미지에 포함된 24시간 OTel JSON과 `precomputed-state` cache를 사용해 `MonitoringSnapshot`, metric series, related logs, risk signals, incident timeline을 반환 |
| `live-otel` | disabled skeleton | `MONITORING_SOURCE_MODE=live-otel` 설정 시 선택 가능하나 `LIVE_OTEL_ENDPOINT`가 없으면 `LIVE_SOURCE_DISABLED`로 명시 실패. 현재 제품 목표는 성공 연결이 아니라 동일 interface와 오류 계약을 유지하는 것이다. 실제 OTLP/Prometheus/Loki adapter 구현은 별도 제품 결정 전까지 non-goal |

공통 성공 응답에는 `sourceMode`, `queryAsOf`, `slot`, `riskSignals`, `evidenceRefs`, `dataFreshness`를 포함합니다. Deterministic monitoring route의 source 오류 응답에는 `code`, `sourceMode`, `queryAsOf`, `requestId`, `recoverable`을 포함하며, Vercel `/api/ai/intelligent-monitoring` batch proxy는 이를 fallback으로 숨기지 않고 pass-through합니다. `/analyze-server`는 기존 generic error boundary를 유지하고, `/incident-report`는 보고서 availability를 위해 가능한 경우 degraded/tool-based fallback을 유지합니다.
...
| `cloud-run/ai-engine/src/services/monitoring/monitoring-data-source.ts` | AI 공통 `MonitoringDataSource` provider와 snapshot/evidence builder |
| `cloud-run/ai-engine/src/tools-ai-sdk/monitoring-tools.ts` | AI SDK monitoring tools (`getMonitoringSnapshot`, metric/log/risk/timeline 조회) |
