...
이 프로젝트는 실 서비스 배포 기준 **`Vercel Pro` 고정비를 제외한 가변 운영 비용 ₩0**를 목표로 설계되었습니다. `Vercel`은 **유일한 유료 예외**이지만, 설계와 운영은 **Free-tier compatible profile** 을 유지해야 하며, Free로 전환해도 핵심 서비스 경로가 깨지지 않아야 합니다. 나머지 인프라는 무료 티어 내에서 동작하도록 **가드레일이 코드 수준에서 강제**됩니다.
...
|--------|------|---------|----------|------|
| **Vercel** | Pro (예외) | Included usage 안에서 Free-compatible profile 유지 | ~5% | 고정 Pro 비용만 허용 |
...
Vercel은 현재 **유일한 유료 예외**로 Pro를 사용합니다. 다만 이는 성능/운영 완충을 위한 선택일 뿐이며, 서비스 성립이 Pro 전용 기능에 의존해서는 안 됩니다. Free로 전환해도 핵심 경로는 유지되고, 악화가 허용되는 것은 응답 지연/빌드 여유/운영 편의성 정도로 제한합니다.
