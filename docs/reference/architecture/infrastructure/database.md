...
> **실측 스냅샷 (2026-06-06)**: knowledge_base=67행, command_vectors=5행, security_audit_logs=425행, approval_history=0행, incident_reports=2행. DB 크기 ~3% (500MB 한도). Free Tier 여유 충분.
...
| 영역 | 현재 기준 |
|---|---|
| Auth/session | Supabase Auth (OAuth: GitHub/Google, OTP) + 자체 guest session |
| AI 운영 규칙 | `system_rules` — AI 카테고리/키/값 동적 설정 (`src/config/rules/loader.ts`) |
| Internal knowledge/KB | `knowledge_base` (67행) + `search_knowledge_text` RPC. 원본 지식은 repo, Supabase는 PostgreSQL Full Text Search index |
| legacy inventory | `command_vectors` (5행, 런타임 미사용), `knowledge_relationships` (런타임 미사용), `knowledge_base.embedding` (cosine path 비활성) |
| Audit/security logs | `security_audit_logs` (425행) — 로그인 감사 이벤트 |
| Approval workflow | `approval_history` (0행) — Cloud Run history/stats 조회 API 호환성만 유지, write path 없음 |
| Incident artifacts | Cloud Run Reporter + 브라우저 세션 아티팩트. `incident_reports` 2행은 2026-01-01 UTC 과거 테스트/레거시 데이터 |
...
## 활성 테이블 현황 (2026-06-06 실측)

| 테이블 | 행 수 | 런타임 사용 | 최적화 과제 |
|--------|:-----:|:----------:|-----------|
| `knowledge_base` | 67 | ✅ 활성 (search_knowledge_text) | embedding 컬럼 제거 (S-3) |
| `system_rules` | — | ✅ 활성 (rules/loader.ts) | — |
| `security_audit_logs` | 425 | ✅ 활성 (login-audit.ts) | 90일 app-level retention cleanup |
| `command_vectors` | 5 | ❌ 미사용 | knowledge_base 잔여 이관 후 삭제 (S-1) |
| `knowledge_relationships` | — | ❌ 미사용 | 삭제 대상 (S-2) |
| `approval_history` | 0 | ⚠️ 조회 API 호환성만 유지 | E-6 완료: write path 없음. `/api/ai/approval/history*` 계약 제거 전까지 DROP 보류 |
| `incident_reports` | 2 | ❌ 미사용 | E-6 완료: current incident artifact path에서 DB read/write 없음. backup 후 삭제 후보 |
...
### 플랫폼 구성
- **PostgreSQL**: 17
- **무료 티어**: 500MB (2026-06-06 실측 ~3% 사용)
- **RLS**: Row Level Security 완전 적용
- **실시간**: 현재 프로젝트에서 WebSocket 실시간 미사용 (Auth/REST API 사용)
- **Schema source**: `supabase/migrations/**` (canonical, 21개 migration 파일)

> ⚠️ **주의**: 아래 `servers`, `server_metrics`, `incidents` DDL은 실제 migration에 **존재하지 않는** 레거시 설계 메모입니다. 실제 서버 메트릭 데이터는 Supabase가 아닌 `public/data/otel-data/` OTel JSON에서 관리됩니다. 실제 테이블 목록은 위의 "활성 테이블 현황" 섹션이 정확합니다.

### system_rules 테이블 (활성)
```sql
-- migration: 20251219101716_create_system_rules_table.sql
...
-- 사용처: src/config/rules/loader.ts (.from('system_rules').select('category, key, value, description, enabled'))
-- RLS: 인증된 사용자 읽기 전용
```

### knowledge_base 테이블 (활성, KRL corpus)
```sql
-- 67행 (2026-06-06), Full Text Search 인덱스 기반
-- search_knowledge_text(text, limit, category) RPC로 조회
-- embedding 컬럼은 존재하나 cosine path 비활성 (S-3 최적화 대상)
```

### security_audit_logs 테이블 (활성, audit)
```sql
-- 425행 (2026-06-06), 계속 증가 중
-- 사용처: src/lib/auth/login-audit.ts
-- retention: 로그인 감사 insert 성공 후 90일 초과 row를 삭제
-- cleanup cadence: 같은 프로세스에서 24시간에 1회
```
...
- `idx_approval_history_status_time`, `idx_approval_history_requested_at`, `idx_approval_history_action_type`, `idx_approval_history_session_id`: read-only compatibility `get_approval_history`/stats access path 보조
