...
| 계층 | 파일 | 설명 |
|------|------|------|
| Breaker 코어 | `src/lib/ai/circuit-breaker.ts` | `AIServiceCircuitBreaker` 클래스 |
| 이벤트 시스템 | `src/lib/ai/circuit-breaker/events.ts` | 상태 전이 이벤트 발행 |
| InMemory 저장소 | `src/lib/ai/circuit-breaker/state-store.ts` | 단일 인스턴스용 메모리 저장 helper |

### 분산 상태 관리 (Redis) — 제거 완료

> **2026-05-20 현황**: Vercel Circuit Breaker의 Redis 분산 상태 저장소는 제거됐다. `src/lib/redis/circuit-breaker-store.ts`, `IDistributedStateStore`, `ensureRedisStateStore()`, `setDistributedStateStore()`는 더 이상 존재하지 않는다. 상태 요약의 `stateStore` 값은 항상 `in-memory`다.
>
> **제거 근거**: Cloud Run AI Engine이 multi-provider key rotation으로 provider 장애를 직접 흡수한다(Part 2 참조). Vercel BFF level CB가 필요한 시나리오는 Cloud Run 서비스 **전체 다운**이며, 이 경우도 각 Vercel 인스턴스의 InMemory CB가 단독으로 차단한다. 분산 CB(Redis)는 비용 대비 효과가 낮아 운영 의존성에서 제외했다.
...
### Vision Agent Provider Degradation

이미지 분석(Vision)은 Gemini 단일 provider로 실행하고, Gemini가 없거나 사용할 수 없을 때만 텍스트 기반 Analyst Agent로 degradation합니다.
...
- Z.AI GLM Vision은 2026-05-20부터 runtime fallback에서 제외합니다.
- 제외 근거: `QA-20260520-0541`에서 GLM fallback이 upstream overload로 HTTP 500을 반환해 답변 품질 검증 전 단계에서 실패했습니다.
...
| Circuit Breaker 분산 저장 | 인스턴스 간 차단 상태 공유 | 2026-05-20 제거 완료 — Vercel CB는 in-memory only |
