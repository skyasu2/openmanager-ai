...
>
> **Version**: 1.3.2 | **Last Updated**: 2026-05-16
...
- **폴백**: Gemini 실패 시 OpenRouter free vision 체인, 그 다음 Z.AI Vision으로 자동 전환
...
- **최대 파일 개수**: 3개 (동시 첨부)
- **총 크기**: 개별 파일 제한 적용
- **폴백**: `Gemini` 설정/사용 불가 시 OpenRouter Vision 모델로 시도하고, OpenRouter까지 실패하면 Z.AI Vision(`glm-4.6v-flash`)을 시도합니다. 모든 Vision provider가 실패할 때만 분석 용도를 유지한 채 텍스트 기반 `Analyst Agent`로 폴백합니다.

> **OpenRouter Vision 권장 구성**: 1차 `google/gemma-3-27b-it:free`,  
> 실패 시 `google/gemma-3-12b-it:free`, `google/gemma-3-4b-it:free`로 폴백(실테스트 기준 정리값). OpenRouter chain 실패 후에는 Z.AI Vision을 마지막 Vision provider로 사용합니다.
...
2. **Vision Agent 가용성**: `GEMINI_API_KEY`, `OPENROUTER_API_KEY`, `ZAI_API_KEY` 중 하나 이상으로 Vision Provider 구성 여부 확인
3. **폴백 처리**: 모든 Vision Provider가 미설정/장애 상태이면 텍스트 기반 `Analyst Agent`로 라우팅
...
# 응답: { "providers": { "gemini": true, "openrouter": true, "zai": true, ... } }
```
...
# Cloud Run / Vercel
vercel env ls production | grep -E "GEMINI|OPENROUTER|ZAI"
```
...
원인: Vision Provider(Gemini/OpenRouter/Z.AI) 미설정 또는 rate limit
```

해결: `GEMINI_API_KEY`, `OPENROUTER_API_KEY`, `ZAI_API_KEY` 환경변수 설정/가용성 확인
