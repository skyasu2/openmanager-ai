...
>
> **Version**: 1.3.3 | **Last Updated**: 2026-05-19
...
- **폴백**: Gemini 실패 시 Z.AI Vision으로 자동 전환
...
- Z.AI `glm-4.6v-flash` fallback: `QA-20260519-0539`에서 `gemini=false` runtime toggle 상태로 production 수동 smoke 확인
- GLM fallback smoke 결과: HTTP 200, `success=true`, `finalAgent=Vision Agent`, `provider=zai`, `modelId=glm-4.6v-flash`, `modeSelectionSource=vision_input`
...
- **최대 파일 개수**: 3개 (동시 첨부)
- **총 크기**: 개별 파일 제한 적용
- **폴백**: `Gemini` 설정/사용 불가 시 Z.AI Vision(`glm-4.6v-flash`)을 시도합니다. 모든 Vision provider가 실패할 때만 분석 용도를 유지한 채 텍스트 기반 `Analyst Agent`로 폴백합니다.
...
2. **Vision Agent 가용성**: `GEMINI_API_KEY`, `ZAI_API_KEY` 중 하나 이상으로 Vision Provider 구성 여부 확인
...
# Cloud Run / Vercel
vercel env ls production | grep -E "GEMINI|ZAI"
```
...
원인: Vision Provider(Gemini/Z.AI) 미설정 또는 rate limit
```

해결: `GEMINI_API_KEY`, `ZAI_API_KEY` 환경변수 설정/가용성 확인
