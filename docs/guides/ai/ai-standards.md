...
|------------------|-----------|----------|--------|
| **Cerebras** | Orchestrator structured routing, opt-in text fallback | preview 모델 변동성과 토큰 정책을 함께 봐야 하므로 tool loop는 기본 비활성 권장 | ⚠️ |
| **Groq** | Supervisor·NLQ·Analyst·Reporter | burst 상황에서 분당 요청 제한과 일일 토큰 한도가 첫 병목이 되기 쉬움 | ⚠️ |
