...
> **최종 갱신**: 2026-05-13 (GitLab-only production deploy alignment)
>
...
| **Metrics Query (Groq Primary / Cerebras Opt-in)** | `npx ts-node scripts/test-cerebras-nlq-local.ts` | 메트릭 조회·수식/통계 계산, Groq primary 대비 fallback 동작 |
...
| **Groq** | Supervisor·Metrics Query primary, Cerebras-first agents fallback | burst 상황에서 분당 요청 제한과 일일 토큰 한도가 첫 병목이 되기 쉬움 | ⚠️ |
