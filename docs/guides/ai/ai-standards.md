...
> **최종 갱신**: 2026-05-13 (GitLab pipeline wait diagnostics alignment)
>
...
- stale release pipeline이 production `resource_group`을 점유하면, 실제 배포/QA가 별도 완료됐거나 사용자가 승인한 경우에만 cancel/clear하고, manual fallback을 사용했다면 최종 보고에 **CI 게이트 스킵** 사실을 명시합니다.
...
- 표준 배포/공개 순서는 아래와 같습니다.
  ...
  - Cloud Run AI Engine 배포: GitLab CI semver tag `deploy_ai_engine` 기본, `cloud-run/ai-engine/deploy.sh`는 명시적 수동 fallback
