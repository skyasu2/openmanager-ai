...
- **Vercel Frontend Git 배포 소스도 GitLab `main`** 입니다. Frontend 자동 배포의 권위 있는 경로는 `git push gitlab main` → Vercel Git Integration 입니다.
- **GitLab CI는 기본 비활성** 입니다. 정책 플래그는 `GITLAB_CI_POLICY=local-docker-only`이며, 정기 파이프라인 대신 로컬 hook/로컬 Docker CI/수동 QA를 우선합니다.
...
- 표준 배포/공개 순서는 아래와 같습니다.
  - Frontend 배포: `git push gitlab main`
