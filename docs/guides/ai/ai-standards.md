...
| :--- | :--- | :--- |
| **Advisor (Groq primary / Cerebras fallback)** | `npx ts-node scripts/test-mistral-advisor-local.ts` | 조치 명령어 제안, 스트리밍 응답 |
...
|------------------|-----------|----------|--------|
| **Cerebras** | Orchestrator·Analyst·Reporter·Verifier primary, Groq-first agents fallback | Qwen/llama runtime chain은 2026-05-27 종료 예정이라 post-deprecation replacement와 계정 entitlement를 함께 추적 | ⚠️ |
| **Groq** | Supervisor·NLQ·Advisor primary, Cerebras-first agents fallback | burst 상황에서 분당 요청 제한과 일일 토큰 한도가 첫 병목이 되기 쉬움 | ⚠️ |
