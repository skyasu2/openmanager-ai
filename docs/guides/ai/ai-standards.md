...
| **Vision (Gemini → OpenRouter → Z.AI Vision)** | `npx ts-node scripts/test-vision-multimodal-fallback.ts` | 이미지 분석, Fallback 메커니즘 |
...
| **Cerebras** | 종료 전 short-context fallback, Analyst-first policy의 첫 후보이나 16K/32K 요구 시 skip | 기본 runtime은 `llama3.1-8b`만 사용하고 2026-05-27 종료 전까지 유지한다. 현재 계정 header 기준 5 RPM / 30K TPM / 2.4K RPD / 1M TPD. Qwen 235B Preview는 runtime 제외 | ⚠️ |
| **Gemini Flash-Lite** | Vision Agent primary | 이미지/멀티모달 요청의 일일 한도와 RPM을 함께 추적해야 함 | ⚠️ |
| **OpenRouter Free** | Vision fallback | free 모델 요청은 계정 크레딧 상태에 따라 RPD가 달라지므로 fallback 전용으로만 사용 | ⚠️ |
