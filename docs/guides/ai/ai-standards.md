...
| **Vision (Gemini → Z.AI Vision)** | `npx ts-node scripts/test-gemini-vision-local.ts` | 이미지 분석, Fallback 메커니즘 |
...
| **Z.AI** | 무료 GLM Flash text/vision fallback, Reporter primary | `glm-4.5-flash`, `glm-4.7-flash`, `glm-4.6v-flash`는 공식 pricing상 무료. Web Search는 유료라 runtime에서 사용 금지. thinking은 요청 패치로 disabled 유지 | ⚠️ |
