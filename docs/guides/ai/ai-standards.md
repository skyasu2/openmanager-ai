...
- Cloud Build: 기본 머신/기본 풀만 사용, 커스텀 machineType 금지
...
- **Frontend production 배포 권한은 GitLab CI `deploy` job** 이 가집니다. `git push gitlab main` 후 GitLab CI `validate -> deploy` 파이프라인이 실행되며, `deploy` job은 `vercel build --prod` + `vercel deploy --prebuilt --prod`로 배포합니다.
- **Vercel Git Integration은 해제** 되어 있습니다. Frontend 자동 배포의 권위 있는 경로는 GitLab CI 파이프라인뿐입니다.
- **GitLab CI는 활성** 상태이며, `.gitlab-ci.yml` validate → deploy 2-stage 파이프라인이 main 브랜치의 코드 변경 push 시 실행됩니다. docs/reports 전용 push는 `changes` 규칙으로 스킵합니다.
- **GitHub (`origin`)는 공개용 code-only snapshot** 입니다. 수동/명시적 동기화 대상으로만 취급하며, private canonical repo와 히스토리가 다를 수 있습니다.
- 현재 GitHub public repo는 **필터링된 최소 공개 이력만 유지**합니다. canonical full history를 공개하지 않으며, release/tag 권위도 갖지 않습니다.
- 현재 GitHub public repo는 **releases/tags가 없고, issues/wiki/projects가 비활성화**된 상태를 기본 운영값으로 봅니다. 즉 협업 허브가 아니라 읽기/분석용 코드 공개면입니다.
- 따라서 AI 에이전트는 **`origin/main`이 canonical branch라고 가정하면 안 됩니다.** push/fetch/rebase 전에 항상 `git remote -v`로 원격 구성을 먼저 확인합니다.
...
- 표준 배포/공개 순서는 아래와 같습니다.
  - Frontend 배포: `git push gitlab main` → GitLab CI `validate` → GitLab CI `deploy`
  - canonical release/tag 반영: `git push gitlab --follow-tags`
...
| **NLQ (Cerebras)** | `npx ts-node scripts/test-cerebras-nlq-local.ts` | `CEREBRAS_MODEL_ID` 기준 SQL 생성 전문성 (기본값: `qwen-3-235b-a22b-instruct-2507`) |
...
### 배포 환경 AI/Redis 의존성 (Production — 무료 티어 엄수)

실제 서비스 경로에서 호출되는 provider와 Redis 의존성입니다. 정확한 공급사 한도 수치는 변동 가능하므로, 이 문서에서는 운영 판단 기준만 유지합니다.

| 프로바이더/서비스 | Agent 역할 | 주요 제약 | 위험도 |
|------------------|-----------|----------|--------|
| **Cerebras** | Supervisor·NLQ·Analyst·Orchestrator | 복잡 질의가 몰리면 일일 토큰/처리량 정책에 먼저 닿기 쉬움 | ⚠️ |
| **Groq** | Reporter (Primary) | burst 상황에서 분당 요청 제한이 첫 병목이 되기 쉬움 | ⚠️ |
| **Mistral** | Advisor (Primary), Embedding | tier 정책 변동성이 있어 배포 전 재확인 필요 | ✅ |
| **Gemini Flash** | Vision Agent | 이미지/멀티모달 요청의 일일 한도 여유가 크지 않음 | ⚠️ |
| **Upstash Redis** | 스트림 저장·재개 인프라·Cache | chunk 수와 resume/cleanup 경로에 따라 명령 수가 빠르게 증가 | ⚠️ |
