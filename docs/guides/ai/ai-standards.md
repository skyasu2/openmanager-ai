...
> **최종 갱신**: 2026-05-16 (AI provider fallback mesh and Free Tier policy alignment)
>
...
- **GitHub public remote의 기본 이름은 `github-public`** 입니다. 공개용 frontend-only snapshot 으로만 취급하며, Vercel에 노출되는 Next.js frontend shell, 공개 정적 데이터, 최소 실행 설정만 공유합니다. `.github/`, `docs/`, `tests/`, `scripts/`, `reports/`, `cloud-run/`, 내부 agent 설정은 공개 snapshot에 포함하지 않습니다. `origin`은 legacy fallback 으로만 허용합니다.
- 현재 GitHub public repo는 **frontend-only 최소 공개 이력만 유지**합니다. canonical full history, Cloud Run AI 엔진, 내부 QA/운영 자산은 공개하지 않으며, release/tag 권위도 갖지 않습니다.
...
- `scripts/ci/runner-health-check.sh`는 로컬 `gitlab-runner` 프로세스와 Docker 가용성만 확인합니다. `exit 0`은 GitLab scheduler, pipeline 생성, runner tag matching, `resource_group` 배정 성공을 의미하지 않습니다.
...
- pipeline 확인 결과가 `note=pipeline_not_terminal_after_wait`이거나 `status=created|pending|running|waiting_for_resource`로 남으면 `npm run gitlab:pipeline:inspect -- --pipeline <id>`로 jobs/resource queue를 확인한 뒤 원인을 보고합니다. `waiting_for_resource`는 runner 미가동으로 단정하지 말고 `resource_group` 점유 여부를 먼저 확인합니다.
- stale release pipeline이 production `resource_group`을 점유하면, 실제 배포/QA가 별도 완료됐거나 사용자가 승인한 경우에만 cancel/clear합니다.
- **로컬 전체 검증 표준 경로는 `npm run ci:local`** 입니다. GitLab CI는 wsl2-docker shell executor로 WSL2에서 직접 실행하므로 ci:local이 동일 환경을 재현합니다. broad change, release 전, 배포 민감 변경에서 push 전에 실행합니다.
- 표준 배포/공개 순서는 아래와 같습니다.
  ...
  - Cloud Run AI Engine 배포: GitLab CI semver tag `deploy_ai_engine` job이 `cloud-run/ai-engine/deploy.sh`를 실행
...
| :--- | :--- | :--- |
| **Advisor (Mistral → Z.AI → Groq → Cerebras)** | `npx ts-node scripts/test-mistral-advisor-local.ts` | 조치 명령어 제안, 스트리밍 응답 |
| **Reporter (Z.AI → Mistral → Groq → Cerebras)** | `npx ts-node scripts/test-groq-reporter-local.ts` | 요약 보고서 작성 품질 |
| **Vision (Gemini only)** | `npx ts-node scripts/test-gemini-vision-local.ts` | 이미지 분석, Gemini 미가용 시 Analyst fallback |
| **Metrics Query (Groq → Z.AI → Mistral → Cerebras)** | `npx ts-node scripts/test-cerebras-nlq-local.ts` | 메트릭 조회·수식/통계 계산, Groq primary 대비 fallback 동작 |
| **전체 Agent API** | `bash scripts/test-agents-api.sh` | Supervisor 라우팅 및 툴 호출 |

> **주의**: Cerebras와 Z.AI는 계정별 접근 가능한 모델/동시성 제한이 다를 수 있습니다. 로컬 테스트는 현재 `.env.local` 모델 ID를 기준으로 수행하고, 일반 인사말보다 도구 호출·구조화 응답·운영 질의 같은 실제 경로로 검증하십시오.
...
|------------------|-----------|----------|--------|
| **Groq** | Supervisor·Metrics Query·Orchestrator primary, 일부 agent fallback | 공식 free limit 기준 Llama 4 Scout는 30 RPM / 1K RPD / 30K TPM / 500K TPD. burst 상황에서 RPD/TPD가 첫 병목이 되기 쉬움 | ⚠️ |
| **Z.AI** | 무료 GLM Flash text fallback, Reporter primary | `glm-4.5-flash`, `glm-4.7-flash`는 text 후보로만 관리. Web Search는 유료라 runtime에서 사용 금지. thinking은 요청 패치로 disabled 유지. `glm-4.6v-flash` Vision fallback은 `QA-20260520-0541`의 overload 재현 이후 runtime에서 제거 | ⚠️ |
| **Mistral** | Advisor primary, 일부 chain secondary/fallback | Experiment/free tier는 workspace 제한 의존. 현재 guard는 보수적으로 관리하며 last-resort 집중을 피하기 위해 일부 경로에 분산 배치 | ✅ |
| **Cerebras** | Round-Robin rotation 동등 후보 (gpt-oss-120b, 65K context) | 2026-05-16 `gpt-oss-120b` 전환 완료. 65K context로 모든 에이전트 eligible. 계정 기준 5 RPM / 2.4K RPD. tool-calling은 `CEREBRAS_TOOL_CALLING_ENABLED=false` 기본값 유지. | ⚠️ |
| **Gemini Flash-Lite** | Vision Agent primary | 이미지/멀티모달 요청의 일일 한도와 RPM을 함께 추적해야 함 | ⚠️ |
| **Upstash Redis** | async Job 상태/진행률/결과, AI 응답 캐시, rate limit, provider quota/session/cache, Langfuse usage guard | Job Queue SSE polling, progress update, Cloud Run quota/session/cache 사용량에 따라 명령 수가 빠르게 증가. 서버 측 stream resume 저장은 2026-05-20 제거됨 | ⚠️ |
