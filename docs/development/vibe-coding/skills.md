...
| `openmanager-stitch-incremental` | 스킬명 명시 또는 작업 매칭 | 기존 UI 증분 개선 (온디맨드, 기본 루프 아님) |
...
| 분류 | 대상 / 스킬셋 | 호출 방식 / 설명 |
|------|-----------|------|
| **Skills** | `openmanager-cloud-run` | 에이전트 자율 진단 (Deploy, free-tier guard, GCP 점검) |
| **Skills** | `openmanager-env-sync` | 에이전트 자율 진단 (preview/production env drift 진단/동기화) |
| **Skills** | `openmanager-qa-ops` | 에이전트 자율 진단 (Vercel + MCP 최종 QA 및 자동 기록) |
| **Skills** | `openmanager-state-triage` | 에이전트 자율 진단 (QA/런타임/배포 상태 증거 기반 원인 분류) |
...
- `openmanager-stitch-incremental`은 현재 상시 워크플로우가 아니라, 명시적인 UI 개선/프로토타이핑 요청이 있을 때만 쓰는 선택적 스킬로 본다.
