...
현재 baseline에 등록된 스킬은 11개입니다. 이 중 10개는 Codex/Gemini 공통 adapter이고, 1개는 Gemini-only overlay입니다.

| Skill | 목적 | Codex/Gemini common adapter | Gemini overlay | Claude adapter |
|-------|------|:--------------------------:|:--------------:|:--------------:|
...
| `gemini-vision-audit` | Gemini Vision 기반 UI 레이아웃/시각 회귀 감사 | - | ✅ | - |
...
- `.gemini/skills`의 Gemini-only overlay가 baseline에 등록되어 있음
