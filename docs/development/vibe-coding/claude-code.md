...
Claude Code는 프로젝트 공유 설정과 로컬 권한 설정을 분리합니다.

```text
~/.claude/settings.json          # 글로벌 설정
.claude/settings.json            # 프로젝트 공통 설정 (Git 추적)
.claude/settings.local.json      # 프로젝트 로컬 권한/활성화 (Git 제외)
.mcp.json                        # 프로젝트 MCP 서버 목록 (Git 추적, 토큰 직접 저장 금지)
.env.local                       # 로컬 시크릿 (Git 제외)
```

| 파일 | 용도 | Git 추적 |
|------|------|:--------:|
| `.claude/settings.json` | Hooks, 출력 스타일 등 공유 설정 | ✅ |
| `.claude/settings.local.json` | 권한, MCP 활성화 등 로컬 설정 | ❌ |
| `.mcp.json` | project-scoped MCP server 목록 | ✅ |
| `.env.local` | 실제 토큰/환경변수 | ❌ |

`.mcp.json`은 더 이상 토큰을 직접 담는 gitignored 파일이 아닙니다. 실제 값은 wrapper script와 환경변수 placeholder를 통해 실행 시점에 주입합니다.
...
`.claude/settings.local.json`에서 로컬 권한과 project MCP 활성화를 관리합니다. 이 파일은 개인 환경 파일이므로 커밋하지 않습니다.
...
| `mcp__server__*` | MCP 서버 전체 | `mcp__supabase-db__*` |
| `mcp__server__tool` | 특정 도구만 | `mcp__github__list_pull_requests` |
| `Skill(name)` | 특정 스킬 | `Skill(code-review)` |
| `WebFetch(domain:*)` | 도메인 허용 | `WebFetch(domain:github.com)` |

### Custom Commands

`.claude/commands/` 디렉토리에 커스텀 슬래시 명령어를 정의합니다.
...
실제 MCP 서버 목록은 tracked `.mcp.json`에 정의합니다. 현재 project MCP는 7개입니다.
...
```json
{
  "mcpServers": {
    "vercel": {
      "command": "bash",
      "args": ["scripts/mcp/start-vercel-mcp.sh"]
    },
    "supabase-db": {
      "command": "bash",
      "args": [
        "scripts/mcp/run-with-project-env.sh",
        "bash",
        "scripts/mcp/start-supabase-mcp.sh"
      ]
    },
    "github": {
      "type": "http",
...
Project MCP 7개가 `Connected`이면 정상입니다. Claude.ai Google Calendar/Drive/Gmail connector가 `Needs authentication`으로 보이는 것은 프로젝트 MCP 문제가 아닙니다.
