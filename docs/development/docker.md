...
- Local Docker CI: `npm run ci:local:docker`가 컨테이너 안에서 검증을 재현
...
│  → WSL2 node/npm/vercel/gcloud      │  → Docker executor 아님
├─────────────────────────────────────┤
│  Supabase local / Local Docker CI   │  Docker Desktop Linux engine
...
| Local Docker CI 보강 검증 | ✅ | `npm run ci:local:docker` |
| AI Engine 코드 수정 후 로컬 테스트 | ✅ | `cd cloud-run && docker compose up --build` |
| AI Engine 프로덕션 배포 | ✅ (GCP Cloud Build 이미지 빌드, 로컬 Docker는 선택 preflight) | `./deploy.sh` |
| Supabase 로컬 실행 | ✅ (Supabase CLI 내부) | `npx supabase start` |

> **판단 기준**: `src/` 중심 Frontend 변경은 Docker 없이 `type-check`/`lint`/`test:quick`/`test:contract`가 기본입니다. `cloud-run/ai-engine/`, Dockerfile, Cloud Run 배포 경계, broad/release 변경이면 Docker preflight 또는 `ci:local:docker`를 추가합니다.
...
GitLab runner는 같은 WSL2 환경에서 `shell` executor로 실행됩니다. 따라서 CI job 자체는 컨테이너 안에서 시작하지 않지만, job script가 `docker ps`, `npm run ci:local:docker`, `cloud-run/ai-engine/scripts/docker-preflight.sh` 같은 명령을 실행하면 WSL Docker CLI가 Docker Desktop engine을 사용합니다.
