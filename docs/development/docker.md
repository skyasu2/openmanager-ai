...
이 프로젝트에서 Docker는 **CI executor가 아니라 보조 런타임**으로 사용됩니다. GitLab CI는 WSL2 self-hosted runner의 `shell` executor가 `node`, `npm`, `vercel`, `gcloud`를 직접 실행하고, Docker는 필요한 경우에만 WSL에서 Docker Desktop Linux engine에 연결해 사용합니다.

주요 사용처는 두 가지입니다.

- Supabase local stack: `npx supabase start`가 내부적으로 Docker 컨테이너를 실행
- AI Engine preflight/build: `cloud-run/ai-engine/Dockerfile` 기준 이미지 빌드와 Cloud Run 배포 전 검증

> **로컬 CI 검증은 Docker 불필요**: GitLab CI가 `wsl2-docker` shell executor로 WSL2에서 직접 실행하므로 `npm run ci:local`이 동일 환경을 재현합니다. `ci:local:docker`(삭제됨)는 다른 환경을 시뮬레이션하므로 제거됐습니다.
...
├─────────────────────────────────────┤
│  GitLab CI / ci:local               │  shell executor (WSL2 직접)
│  → WSL2 node/npm/vercel/gcloud      │  → Docker 불필요
├─────────────────────────────────────┤
│  Supabase local                     │  Docker Desktop Linux engine
│  → WSL2 docker CLI                  │  → /var/run/docker.sock
├─────────────────────────────────────┤
│  AI Engine (Hono + AI runtime)      │  Dockerfile + Cloud Build
│  → docker preflight / compose       │  → Cloud Run (프로덕션)
└─────────────────────────────────────┘
```
...
| GitLab CI validate/deploy job 실행 | ❌ | WSL2 shell executor |
| 로컬 전체 검증 (ci:local) | ❌ | `npm run ci:local` |
| AI Engine 코드 수정 후 로컬 테스트 | ✅ | `cd cloud-run && docker compose up --build` |
| AI Engine 배포 전 이미지 빌드 검증 | ✅ (선택) | `cd cloud-run/ai-engine && npm run docker:preflight` |
| AI Engine 프로덕션 배포 | ✅ (GCP Cloud Build 이미지 빌드) | `./deploy.sh` |
| Supabase 로컬 실행 | ✅ (Supabase CLI 내부) | `npx supabase start` |

> **판단 기준**: `src/` 중심 Frontend 변경은 Docker 없이 `npm run ci:local`이 기본입니다. `cloud-run/ai-engine/`, Dockerfile, Cloud Run 배포 경계 변경이면 `docker:preflight`를 추가합니다.
...
수동 `deploy.sh`는 기본적으로 로컬 Docker build-only preflight를 먼저 실행하고, GitLab CI `deploy_ai_engine`은 중복 빌드를 피하려고 `LOCAL_DOCKER_PREFLIGHT=false`로 실행합니다. Cloud Build 자체는 로컬 Docker 없이 동작합니다.
...
Docker Desktop이 Windows에서 Docker Engine을 실행하고, WSL 2에 소켓(`/var/run/docker.sock`)을 공유합니다. WSL 터미널에서 `docker` 명령을 실행하면 이 소켓을 통해 Windows 측 Docker Desktop Linux engine에 명령이 전달됩니다.

GitLab runner는 같은 WSL2 환경에서 `shell` executor로 실행됩니다. 따라서 CI job 자체는 컨테이너 안에서 시작하지 않지만, job script가 `docker ps`, `cloud-run/ai-engine/scripts/docker-preflight.sh` 같은 명령을 실행하면 WSL Docker CLI가 Docker Desktop engine을 사용합니다.
...
│   ├── cloudbuild.yaml         # Cloud Build reference/alternate config
│   ├── src/                    # TypeScript 소스
...
GEMINI_API_KEY_SECONDARY=your-key
MISTRAL_API_KEY=your-key
SUPABASE_URL=https://xxx.supabase.co
SUPABASE_SERVICE_ROLE_KEY=your-key
```

`GEMINI_API_KEY` / `GEMINI_API_KEY_PRIMARY`가 AI Engine runtime parser의 우선 키입니다. 기존 `GOOGLE_API_KEY` 값이 있으면 새 로컬 Compose 실행에서는 `GEMINI_API_KEY_PRIMARY`로 옮겨 적습니다.

로컬 이미지 라벨에 실제 패키지 버전을 넣고 싶으면 `APP_VERSION`을 명시합니다. 기본값은 stale 숫자를 피하기 위해 `local`입니다.

```bash
APP_VERSION=$(node -p "require('./ai-engine/package.json').version") docker compose up --build
```
...
로컬 Docker는 Supabase stack, GitHub MCP, AI Engine preflight가 같이 사용하므로 삭제 전에 반드시 현재 실행 중인 컨테이너와 volume 링크를 확인합니다. 주기 점검은 비파괴 감사부터 시작합니다.
