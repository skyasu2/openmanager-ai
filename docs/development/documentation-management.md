...
2. 활성 문서 예산: 총 90개(`docs/archived/` 제외)
   ...
   - 파일 길이 기준: ≤600줄 정상, 601~1200줄 경고(중요 내용 앞 배치), >1200줄 분할 권고 — 코드 max-lines 규칙이 아닌 LLM "Lost in the Middle" 인지 손실 방지 기준 (TACL 2024)
...
| 변경 유형 | 같이 확인할 문서/명령 |
|---|---|
| API route 추가/삭제 | `npm run docs:api:endpoints`, [API 설계 경계](../design/README.md#api-설계-경계) |
| AI stream/job/tool schema 변경 | [AI Agent 설계 경계](../design/README.md#ai-agent-설계-경계), [API Endpoints](../reference/api/endpoints.md) |
...
| Docs-as-Code | PR/버전관리/CI 게이트 | lint/link/check 파이프라인 존재 | 변경 문서 메타데이터 hard gate | P0 |
| 스타일 일관성 | 스타일 가이드 기반 작성 | markdownlint + 내부 규칙 | 문서별 owner/책임 경계 약함 | P1 |
| 메타데이터 표준 | owner/status/date/canonical 권장 | 4필드 필수 적용 | ✅ 완료 | P0 |
| 제목 규칙 | 유형별 일관된 패턴 | **신규 (2026-05-20)** 파일명·H1 규칙 추가 | ADR 외 패턴 미정립 → 규칙 신설 | P0 |
| 문서 크기 기준 | AI 인지 손실 방지 (LLM "Lost in the Middle") | **신규 (2026-05-20)** ≤600/1200 기준으로 교체 | 기존 800/1500은 코드 규칙 차용, 근거 없음 | P0 |
| 문서 수명주기 | stale 정책 + 아카이브 | 90일 stale 감지 존재 | stale는 경고 중심, 자동화 후속 부족 | P1 |
| 중복 방지 | canonical link + merge-first | 중복 후보 탐지 있음 | 주기적 내용 비교 분석 필요 | P1 |
| AI 참조성 | 중요 내용을 문서 앞 1/3에 배치 | **신규 (2026-05-20)** 규칙 추가 | 기존 문서 소급 적용 필요 | P1 |
...
- "Lost in the Middle" — LLM 문서 길이 기준 근거 (TACL 2024): <https://arxiv.org/abs/2307.03172>
