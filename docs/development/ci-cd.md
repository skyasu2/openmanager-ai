...
> GitLab canonical + GitLab CI frontend/ai-engine validate + main frontend/ai-engine deploy + frontend/ai-engine smoke 운영 가이드
...
| `smoke` | shared | main + frontend 변경 또는 main + ai-engine/CI smoke 경로 변경 |
...
코드 변경 → pre-commit / pre-push / 필요 시 `npm run ci:local:docker`
        ...
        → GitLab CI validate(frontend) (`type-check` + `lint:ci` + `test:quick` + `test:contract`)
        → GitLab CI validate(ai-engine) (`cd cloud-run/ai-engine && npm run type-check && npm run test`, 변경 시만)
        → GitLab CI deploy(frontend) (`vercel build --prod` + `vercel deploy --prebuilt --prod`, frontend 변경 시만)
        → GitLab CI deploy(ai-engine) (`cd cloud-run/ai-engine && LOCAL_DOCKER_PREFLIGHT=false bash deploy.sh`, ai-engine 변경 시만)
        → GitLab CI post-deploy smoke(frontend) (`/`, `/validation`, `/api/version`, frontend 변경 시만)
        → GitLab CI post-deploy smoke(ai-engine) (`/health`, `/warmup`, `/monitoring`, ai-engine 변경 시만)
        → Vercel production
...
| 빌드 위치 | Vercel build 인프라 | GitLab CI `deploy` job의 shared runner |
...
| preview/prod 자동화 | Vercel branch tracking 중심 | GitLab branch/MR validate + main deploy 중심 |
...
- 실제 역할 분리는 다음과 같습니다.
  - `validate(frontend)`, `validate_ai_engine`: `wsl2-docker` self-hosted runner
  - `deploy(frontend)`: GitLab shared runner에서 `vercel build --prod` 실행
  - `post_deploy_smoke(frontend)`: GitLab shared runner
- 따라서 현재 방식의 정확한 표현은 **"self-hosted validate + shared-runner prebuilt deploy"** 입니다.
...
## 현재 저장소/배포 토폴로지 (2026-04-03)

- **GitLab private (`gitlab`)**: canonical development repo
- **Vercel Frontend**: GitLab CI `deploy` job이 shared runner에서 `vercel build` + `vercel deploy --prebuilt --prod`로 production 배포
- **Cloud Run AI Engine**: GitLab CI `deploy_ai_engine` job이 main `cloud-run/ai-engine/**` 변경 시 production 배포를 수행하고, `post_deploy_ai_engine_smoke`는 배포 뒤 또는 smoke 스크립트/CI 설정 변경 시 현재 production service를 대상으로 `/health`, `/warmup`, `/monitoring` 기준 최소 smoke를 수행
- **GitLab CI**: 활성 (`branch/MR validate(frontend + ai-engine) -> main frontend/ai-engine deploy -> main frontend/ai-engine smoke`)
...
- **GitHub public (`github-public`, `origin` legacy fallback)**: code-only snapshot, 수동 동기화 전용
...
1. Frontend 배포
- feature branch / Merge Request 단계에서 GitLab CI `validate` (`test:contract` 포함)
- `git push gitlab main`
- GitLab CI `validate`
...
- frontend-only 변경이면 Cloud Run deploy는 실행되지 않음
...
|---|---:|---|---|---|
| wsl2-docker self-hosted runner | GitLab quota 0 | 중간 | 높음 | 활성 (`validate`, `validate_ai_engine`) |
...
판단 기준:
- `validate`, `validate_ai_engine` job은 `tags: [wsl2-docker]`가 붙은 self-hosted runner에서 실행되어 GitLab compute minutes를 소모하지 않습니다.
- `deploy` job은 태그 없이 GitLab.com shared runner에서 실행되며, frontend 변경이 있을 때만 현재 기준 약 4분 내외의 compute quota를 사용합니다.
...
6. GitLab CI `deploy`는 main frontend 변경에서 shared runner가 `vercel build --prod` + `vercel deploy --prebuilt --prod` 수행
7. GitLab CI `post_deploy_smoke`는 main frontend 변경에서 shared runner가 `/`, `/validation`, `/api/version` 저비용 smoke 확인
8. production `deploy`는 `resource_group: production`으로 직렬화되어 연속 push에서도 동시 실행되지 않음
9. 외부 pull까지 차단해야 할 때만 `CI_DOCKER_PULL_POLICY=never` 사용
...
- `deploy` runner: GitLab.com shared runner (태그 없음, frontend 변경 시만)
...
즉, **현재는 self-hosted frontend validate + self-hosted ai-engine validate + shared frontend deploy + shared frontend smoke + shared ai-engine deploy + local Docker CI 보강 검증** 구성이 기본 운영값입니다.

### 비용 정책

- **GitLab CI**: 활성. 무료 400분/월 예산을 넘기지 않도록 frontend/ai-engine validate는 self-hosted, frontend deploy/smoke는 shared runner, ai-engine deploy는 변경 시에만 실행
...
> 현재 primary delivery path는 `feature branch / MR validate(frontend + ai-engine)` → `git push gitlab main` → GitLab CI `validate(frontend + ai-engine)` → GitLab CI `deploy(frontend)` / `deploy_ai_engine` → GitLab CI `post_deploy_smoke` → production 입니다.
