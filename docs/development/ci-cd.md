...
- 현재 GitLab native status gate가 없으므로 **automerge는 기본 비활성** 상태를 유지합니다.
...
- 현재 GitLab canonical 경로에는 GitLab native required status check가 없습니다.
- 따라서 patch라도 자동 병합까지 열면 검증 없이 `main`에 들어갈 수 있습니다.
