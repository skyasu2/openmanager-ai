...
현재 운영 기준은 **GitLab canonical repo + GitLab CI branch/main validate(frontend + ai-engine) + semver tag deploy(frontend + ai-engine) + post-deploy smoke** 입니다. 로컬 사전 검증은 `npm run ci:local` (GitLab shell executor 동등 직접 실행)을 사용합니다. GitHub는 이제 **frontend-only public snapshot** 으로만 사용하며, CI/CD·배포·릴리즈 권위가 없습니다.
...
flowchart LR
    ...
    subgraph validate["validate - self-hosted runner (분 소진 0)"]
        vci[validate_ci_config]
        vfe[validate_frontend]
        vsb[validate_storybook_smoke]
...
| Stage | Runner | 트리거 조건 |
|-------|--------|------------|
| `validate_ci_config` | self-hosted `wsl2-docker` (분 소진 0) | `.gitlab-ci.yml` 변경 push, lightweight semantic guard |
| `validate` | self-hosted `wsl2-docker` (분 소진 0) | branch/MR/main 프론트엔드/공용 코드 변경 push |
| `validate_storybook_smoke` | self-hosted `wsl2-docker` (분 소진 0) | Storybook/UI/package 관련 변경 push |
...
```
코드 변경 → pre-commit / pre-push / 필요 시 `npm run ci:local`
        → `git push gitlab main`
        → GitLab CI validate_ci_config (`.gitlab-ci.yml` 변경 시 lightweight semantic guard)
        → GitLab CI validate(frontend) (`type-check` + `lint:ci` + `test:quick` + `test:contract`)
        → GitLab CI validate_storybook_smoke (`storybook:smoke`, UI/Storybook 관련 변경 시)
...
릴리즈 배포 → `./scripts/release/publish.sh patch|minor|major`
          ...
          → release metadata commit의 main validate pipeline은 생략
...
- **GitLab CI**: 활성 (`branch/MR/main validate_ci_config + validate(frontend + Storybook smoke + ai-engine) -> semver tag deploy(frontend + ai-engine) -> semver tag smoke`)
- **`.gitlab-ci.yml`**: 최소 파이프라인으로 유지. docs/reports 전용 push는 `changes` 규칙으로 CI 스킵
- **CI config-only push**: `.gitlab-ci.yml` 단독 변경은 `validate_ci_config`만 실행합니다. 앱/AI/UI/번들/의존성 job은 관련 코드 경로가 함께 바뀔 때만 실행해 단일 self-hosted runner 큐를 줄입니다.
- **Release metadata commit**: `chore(release):` branch push는 main validate를 생략하고, 같은 push의 semver tag pipeline이 deploy/smoke 권위를 가짐
...
- **Local CI**: `npm run ci:local` 을 broad/release 전 전체 검증 경로로 사용 (GitLab shell executor와 동일 환경)
...
- `cloud-run/ai-engine/package.json` / `package-lock.json`의 version 필드만 바뀐 릴리스 메타데이터 변경은 `scripts/ci/should-deploy-ai-engine.sh`가 Cloud Run 재배포를 생략한다.
...
`npm run ci:local`은 GitLab CI `validate` + `validate_ai_engine` job과 동일한 명령을 WSL2에서 직접 실행합니다. GitLab CI가 `wsl2-docker` shell executor(Docker 없이 WSL2 Node 직접 사용)이므로 이 스크립트가 실제 CI 환경을 그대로 재현합니다.

```bash
# 표준: type-check + lint + test:quick + test:contract + AI Engine 검증
npm run ci:local

# 빠른 확인: AI Engine·계약 테스트 제외
npm run ci:local:quick

# AI Engine 배포 전 Docker 이미지 빌드 검증 (별도)
cd cloud-run/ai-engine && npm run docker:preflight
```

운영 원칙:
- `.gitlab-ci.yml`은 branch/MR `validate_ci_config + validate(frontend + ai-engine)` → semver tag `deploy(frontend + ai-engine)` → `post_deploy_smoke` 최소 파이프라인으로 유지합니다.
- `.gitlab-ci.yml`만 바뀐 push는 lightweight `validate_ci_config`만 실행합니다. CI 설정 변경과 앱/AI 코드 변경이 함께 있으면 해당 코드 경로의 heavy validate job도 같이 실행됩니다.
- branch에 열린 Merge Request가 있으면 push pipeline 대신 MR pipeline만 유지해 중복 실행을 줄입니다.
- `post_deploy_smoke`는 `/`, `/login`, `/api/version`만 확인합니다.
- broad change, release 전, 배포 민감 변경에서는 pre-push hook만으로 끝내지 말고 `npm run ci:local`을 추가로 실행합니다.
- docs/reports 전용 push는 `.gitlab-ci.yml`의 `changes` 규칙 + commit-type Layer 1으로 CI가 스킵됩니다.
- `chore(release):` 메타데이터 commit은 release script의 local consistency check와 semver tag pipeline으로 검증되며, self-hosted runner 경합을 줄이기 위해 branch validate를 생략합니다.
- 로컬 설치 트리 전체 재구성이 필요하면 `npm run clean:all`을 사용합니다.
...
| ci:local (Shell 직접) | GitLab quota 0 | 없음 | GitLab native status 없음 | broad/release 전 표준 경로 |
...
2. broad change, release 전, 배포 민감 변경에는 `npm run ci:local`을 push 전에 추가
...
- `validate_storybook_smoke` runner: `wsl2-docker` self-hosted runner
...
### Storybook CI Guardrail

Storybook은 기본 branch/MR validate에 full build를 상시 포함하지 않고, UI/Storybook 관련 변경에서 빠른 smoke job만 실행합니다.

`validate_storybook_smoke` 조건:
- 실행 명령: `npm run storybook:smoke`
- runner: `wsl2-docker`
- timeout: 8 minutes
- 실행 경로: `.storybook/**/*`, `scripts/storybook/**/*`, `config/testing/vitest.config.storybook.ts`, `src/**/*.stories.ts(x)`, `src/components/**/*`, `src/app/**/*`, `package.json`, `package-lock.json`
- QA 기록 커밋(`test(qa): ...`)은 Storybook smoke를 생략합니다.

Full Storybook build는 기본 branch/MR validate에 넣지 않습니다. 아래 조건에서 수동으로 실행합니다.
...
수동 실행 조건:
- Storybook framework/addon/config를 변경한 경우
...
- **ci:local**: broad/release 전 전체 검증에 사용 (`npm run ci:local`, GitLab shell executor 동등)
...
> 현재 primary delivery path는 `pre-commit / pre-push / 필요 시 ci:local` → `git push gitlab main`으로 validate → `./scripts/release/publish.sh patch|minor|major` + semver tag push로 deploy/smoke → production 입니다. branch/MR validate는 선택 경로로 유지합니다.
...
- `post_deploy_ai_engine_smoke`는 `deploy_ai_engine`가 같은 파이프라인에 있을 때는 그 job 이후에 대기하고, smoke 스크립트가 바뀐 경우에는 현재 production service를 직접 점검합니다.
- `cloud-run/ai-engine/package.json` / `package-lock.json`의 version 필드만 바뀐 release metadata diff는 Cloud Build/Cloud Run 재배포를 유발하지 않습니다. AI Engine 실제 코드, Dockerfile, config, dependency 내용, smoke script가 바뀌면 기존처럼 배포합니다.
