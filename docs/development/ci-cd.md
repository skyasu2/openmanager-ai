...
> 현재 primary delivery path는 `pre-commit / pre-push / 필요 시 ci:local:docker` → `git push gitlab main` → GitLab CI `validate(frontend + ai-engine)` → GitLab CI `deploy(frontend)` / `deploy_ai_engine` → GitLab CI `post_deploy_smoke` → production 입니다. branch/MR validate는 선택 경로로 유지합니다.
