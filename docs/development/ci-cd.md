...
| `validate_storybook_smoke` | self-hosted `wsl2-docker` (분 소진 0) | Storybook/UI/package/CI 관련 변경 push |
...
- **GitLab CI**: 활성 (`branch/MR/main validate(frontend + Storybook smoke + ai-engine) -> semver tag deploy(frontend + ai-engine) -> semver tag smoke`)
...
운영 원칙:
- `.gitlab-ci.yml`은 branch/MR `validate(frontend + ai-engine)` → semver tag `deploy(frontend + ai-engine)` → `post_deploy_smoke` 최소 파이프라인으로 유지합니다.
...
- runner: `wsl2-docker`
- timeout: 8 minutes
- 실행 경로: `.storybook/**/*`, `scripts/storybook/**/*`, `config/testing/vitest.config.storybook.ts`, `src/**/*.stories.ts(x)`, `src/components/**/*`, `src/app/**/*`, `package.json`, `package-lock.json`, `.gitlab-ci.yml`
