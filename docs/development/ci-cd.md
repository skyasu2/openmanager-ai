...
> GitLab canonical + GitLab CI frontend/ai-engine validate + main frontend/ai-engine deploy + frontend smoke 운영 가이드
...
코드 변경 → pre-commit / pre-push / 필요 시 `npm run ci:local:docker`
        ...
        → GitLab CI post-deploy smoke (`/`, `/validation`, `/api/version`, frontend 변경 시만)
        → Vercel production
...
- **Cloud Run AI Engine**: GitLab CI `deploy_ai_engine` job이 main `cloud-run/ai-engine/**` 변경 시 production 배포
- **GitLab CI**: 활성 (`branch/MR validate(frontend + ai-engine) -> main frontend/ai-engine deploy -> main frontend smoke`)
