...
| `MISTRAL_MODEL_ID` | Cloud Run AI Engine의 Mistral text fallback 모델 override. Vercel `/api/ai/artifact-intent`는 고정 classifier 모델(`ministral-3b-latest`)을 사용하므로 이 값을 읽지 않음 | `mistral-small-latest` |
...
| `UPSTASH_REDIS_OPERATION_TIMEOUT_MS` | 공통 Redis 작업 타임아웃 (ms) | `1200` |
| `UPSTASH_REDIS_FAST_TIMEOUT_MS` | fast read 경로 타임아웃 (ms) | `800` |
| `UPSTASH_REDIS_STANDARD_TIMEOUT_MS` | 일반 Redis 경로 타임아웃 (ms) | `1000` |
