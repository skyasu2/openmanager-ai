...
> Canonical: docs/archived/ai-assistant-initial-design-comparison.md
> Tags: ai,assistant,architecture,frontend,backend,comparison

**분석 기준일**: 2026-05-07
**현재 구현 기준**: OpenManager AI v8.11.x, Vercel Frontend + Cloud Run AI Engine
...
- artifact workspace/schema registry, route catalog 정리, provider-native reasoning policy가 완료되어 핵심 결론이 정본 문서에 흡수되면 이 문서는 `docs/archived/` 이동 후보가 됩니다.

**현재 상태 판정**:

- 현재 구현은 여전히 **Option A 기반**이지만, Option A의 이름을 더 이상 "multi-agent first"로 이해하면 안 된다. 2026-05-03~05 작업 이후 실제 기준은 **deterministic/single-first + conditional multi-agent escalation**이다.
- M1~M7과 AI Streaming UI S1~S3가 완료되어 단순 read-only contract 단계를 넘어 **deterministic fact boundary + stream step visibility** 단계에 들어갔다.
...
- M6의 `/api/ai/ask` wrapper-only facade는 2026-05-07 dead-code cleanup에서 제거됐고, M7은 `MonitoringFactPack`, retrieval recall guard, provider freshness guard를 deterministic test 범위로 추가했다.
- 남은 핵심 제품 과제는 artifact workspace/schema registry, route catalog 정리, provider-native reasoning capability policy 정밀화다.

---

## 0. 고정 전제 조건 (변경 없음)

비교의 전제 조건이다. 이 조건을 충족하지 못하는 대안은 현재 포트폴리오 기준의 검토 대상에서 제외한다.
...
| BFF 목표 | 기본 채팅 entrypoint는 `/api/ai/supervisor/stream/v2`로 유지한다. 새 route surface는 즉시 늘리지 않고, `AssistantPlan`/`AssistantResult` 공통 metadata부터 수렴한다. |
...
이 문서의 점수와 채택 판단은 **현재 포트폴리오/무료 티어 트랙** 기준이다. 실제 상용 제품처럼 하나의 제품 서버 또는 단일 managed platform에 기능을 모으고, AI API를 유료로 안정적으로 사용할 수 있다는 전제에서는 별도 평가가 필요하다.

| 평가 트랙 | 전제 | 설계 판단 |
|-----------|------|-----------|
| 현재 포트폴리오 트랙 | Vercel + Cloud Run + Supabase + Redis + 여러 무료/저비용 provider에 분산. Free Tier, cold start, provider quota, CI 비용 방어가 핵심 제약 | Option A 유지 + C/E 원칙 흡수가 합리적이다. multi-provider fallback과 job/proxy 복잡도는 포트폴리오 제약을 감당하기 위한 비용이다. |
| 상용 제품 트랙 | 단일 제품 backend 또는 통합 managed platform, paid LLM API, 안정적인 quota/SLA, live telemetry ingestion, 장기 저장, 권한/감사/incident workflow 가능 | Option A를 그대로 재사용한다고 단정하면 안 된다. Enterprise AIOps, managed agent platform, durable workflow, artifact-first/product workspace, paid structured-output provider 중심 설계를 다시 점수화해야 한다. |

따라서 "Option A를 다시 선택한다"는 결론은 현재 포트폴리오 트랙의 결론이다. 상용 제품 트랙에서는 Cloud Run/Vercel 분리, multi-provider 무료 fallback, Redis/Cloud Tasks job queue가 오히려 불필요한 복잡도가 될 수 있다.

---

## 1. 결론 요약

현재 포트폴리오 제약에서 처음부터 설계한다면 **Option A(현재 채택 구조)**를 다시 선택한다. 단, 지금 바로 개선할 수 있는 항목들과 다음 단계에서 흡수할 두 가지 원칙이 있다.
...
- `routeDecision` metadata, artifact 표준 필드, provider freshness는 대체 설계가 아니라 현재 Option A에 흡수할 개선 항목이다.

**지금 바로 개선 가능한 것**

| 개선 | 방법 | 현재 구현 상태 |
|------|------|----------------|
| BFF surface 수렴 | 기존 streaming/job/artifact route가 같은 `AssistantPlan`/`AssistantResult` metadata를 쓰게 하고, 사용되지 않는 route surface는 제거한다. | M6 wrapper facade는 2026-05-07 dead-code cleanup에서 제거됐다. 기본 채팅 경로는 `/api/ai/supervisor/stream/v2`로 수렴했다. |
...
### 1.1 현재 방식과 대안 방식 비교 결론

사용자-facing으로 설명할 때는 아래 표가 가장 정확하다. 이 표는 새 설계안이 아니라, §4의 상세 대안 비교를 현재 구현 상태 기준으로 압축한 결론이다.

| 방식 | 핵심 구조 | 장점 | 주요 한계 | OpenManager 현재 판단 |
|------|-----------|------|-----------|-----------------------|
| 일반 LLM 챗봇 | 서버 데이터를 prompt에 넣고 LLM이 판단·생성 | MVP가 빠르고 자유 질의에 강함 | 수치/임계치 판단이 흔들릴 수 있고 회귀 테스트가 어렵다 | 서버 모니터링 운영 판단에는 부적합 |
| Multi-agent first | 대부분 요청을 Orchestrator/Agent 협업으로 처리 | 복잡 질의 데모와 specialist handoff에 강함 | 단순 조회까지 비용·지연·drift가 증가한다 | 현재 기준으로는 과도함. multi-agent는 escalation path로 축소 |
| Rule-only / dashboard-only | LLM 없이 코드와 UI만 사용 | 가장 빠르고 재현 가능함 | 자연어 질의, 설명, 보고서 artifact 경험이 약함 | 단독 제품 방향으로는 부족 |
| Heavy RAG / GraphRAG | 검색·지식 그래프 중심으로 답변 grounding 강화 | 긴 문서/지식 질의에 강함 | 운영비, 인프라, eval 복잡도가 크고 Free Tier와 충돌하기 쉽다 | 현재는 Knowledge Retrieval Lite + evidence guard 유지 |
| Managed agent platform | provider-native agent runtime, tracing, guardrail, hosted tool 사용 | 빠른 제품화와 표준 기능 흡수에 유리 | provider lock-in, 비용, multi-provider fallback 약화 | core 대체는 보류. guardrail/eval/tracing 원칙만 흡수 |
| Enterprise AIOps | Datadog/New Relic/Dynatrace식 실시간 telemetry + AI | 실제 운영 제품 수준 | live ingest, 장기 저장, 권한, alert history, incident workflow 필요 | 현재 포트폴리오/free-tier 범위 밖. 실제 서버 변경 권한 없이 자율 조치를 흉내 내면 데모성 자동화가 된다 |
| OpenManager 현재 방식 | 운영 의사결정 AI 어시스턴트 + deterministic/single-first runtime | 정확성, 비용, 속도, 테스트 가능성의 균형이 좋음 | 도메인별 FactPack/schema/eval 설계가 필요하고 실제 조치는 운영자 실행/승인을 전제로 한다 | 서버 모니터링 도메인에는 가장 적합 |

정리하면 OpenManager의 범용 분류는 **운영 의사결정 AI 어시스턴트**다. 서버 모니터링은 적용 도메인이고, 구현 방식은 **tool-augmented LLM application with a deterministic decision layer**다. 핵심 운영 수치와 판정은 deterministic fact boundary가 계산하고, LLM은 설명·요약·보고서·조치안 초안을 맡는다. 실제 서버 변경은 하지 않으며 operator 승인/실행을 전제로 한다.

영문 공개 문서에서는 **Operational Decision Support AI Assistant**를 기본 표기로 사용한다. 도메인을 함께 드러낼 때만 **Server Monitoring / Observability AI Assistant**로 좁혀 쓰고, `AIOps`는 "AIOps-adjacent" 또는 "AIOps-inspired" 수준으로 제한한다.

### 1.2 Vercel AI SDK 사용 경계
...
| 리스크 | 영향 | 관찰 포인트 |
|--------|------|-------------|
| Frontend와 backend 라우팅 중복 | M5 planner shadow와 M6 wrapper facade 실험으로 drift 관측은 가능해졌지만, 실제 authority는 아직 frontend/BFF/Cloud Run에 분산되어 있다. | `/api/ai/ask` facade는 2026-05-07 제거됐다. authority 이전은 drift/latency/free-tier 기준 통과 후 별도 gate로 제한한다. |
| API surface가 넓음 | `artifact-intent`, `incident-report`, `intelligent-monitoring`, `jobs`, `supervisor` 등 경로가 많아 contract drift 가능성이 있다. | 새 facade를 늘리기보다 기존 route contract 정리와 route catalog 정리를 우선한다. |
...
**분리 자체는 올바른 결정이었다**: Vercel에 모든 것을 넣으면 route별 duration, 비용, streaming proxy 안정성 제약을 강하게 받기 쉽고, Cloud Run에 모든 것을 넣으면 SSR/CDN/edge 이점을 포기해야 한다. 분리는 맞지만, 분리로 인한 복잡도를 §7의 개선 방향(`AssistantPlan` 통일, route catalog 정리, Planner를 Cloud Run 집중)으로 줄이는 것이 과제다.
...
**업계 Observability AI copilot 비교**:

| 제품 | 공식 문서상 동작 방식 | 사용하는 근거 데이터 | OpenManager에 주는 시사점 |
|------|----------------------|----------------------|---------------------------|
| Datadog Bits AI SRE | 조사 루프는 observation, reasoning, action을 반복한다. 가설을 만들고 telemetry를 조회해 검증/반증하며, 근거가 부족하면 inconclusive로 끝낸다. | Metrics, APM traces, logs, dashboards, events, change tracking, source code, Watchdog, RUM, network path, DB monitoring, profiler 등 | "AI가 그냥 답한다"가 아니라 tool이 telemetry를 조회하고 evidence-backed conclusion을 만든다. OpenManager의 `MonitoringFactPack`도 같은 boundary를 유지해야 한다. |
| New Relic AI | 사용자의 자연어 질문을 observability assistant가 해석하고, 제품 문서/example query/사용자 telemetry data 같은 입력을 사용해 응답한다. 공식 문서는 AI 응답을 action 전 human verification 대상으로 둔다. | New Relic 계정의 telemetry data, 제품 문서, example query library | AI 응답은 실행 명령이 아니라 검증 가능한 suggestion이다. OpenManager도 operator advisory assistant로 포지셔닝하고, evidence/fact boundary를 UI에 노출해야 한다. |
| Elastic Observability AI Assistant | knowledge base에 semantic search를 수행하고 top result를 LLM context로 넘기는 RAG를 명시한다. Runbook, GitHub issue, 내부 문서, Slack 메시지를 KB에 넣을 수 있다. | Elasticsearch/Kibana context, internal knowledge base, runbooks, GitHub/Jira/Slack/Confluence류 content connector | 조직별 runbook/incident 지식은 RAG/KB가 맞다. 단, OpenManager는 Heavy RAG가 아니라 Knowledge Retrieval Lite + evidence guard로 비용과 범위를 제한한다. |
| Dynatrace Davis / Dynatrace Intelligence RCA | Davis event와 logs, metrics, traces를 함께 쓰고, topology/transaction/code-level context로 root cause와 impact를 계산한다. 시간 상관만으로는 부족하다고 명시한다. | Davis events, logs, metrics, traces, topology, transaction, code-level information | root cause ranking은 LLM 문장 생성이 아니라 topology correlation과 deterministic/event correlation이 중심이다. OpenManager는 causal graph 전체 대신 tier/topology + risk signal ranking으로 축소한다. |
| Grafana Sift | labelset을 기준으로 Prometheus/Loki/Tempo datasource를 조회해 investigation scope를 잡는다. scope가 너무 넓어 noisy하면 investigation을 실행하지 않는다. | Prometheus metrics, Loki logs, Tempo traces, panel/explore/dashboard label context | 넓은 자연어 질문을 무조건 LLM에 넣지 않고 scope narrowing/clarification/skip이 필요하다. OpenManager의 evidence 부족 응답과 query scope selector 방향과 맞다. |

**핵심 확인**: 업계 레퍼런스는 모두 **"observability data/tool/query engine + evidence/scope guard + LLM narration"** 패턴을 사용한다. LLM이 메트릭을 직접 "판단"하지 않는다. OpenManager의 `precomputed-state`/`MonitoringFactPack` → deterministic tool → LLM explanation 실질 동작은 이 업계 표준과 일치한다.

**RAG 판단**: RAG는 Elastic처럼 runbook, issue, 내부 문서, Slack 지식 같은 **조직 운영 지식**을 연결할 때 필요하다. 반대로 CPU 90%, alert count, topology risk 같은 **현재 관측 사실**은 RAG가 아니라 monitoring data source/tool/fact pack이 책임져야 한다. 따라서 user-facing RAG On/Off 토글보다 internal evidence guard, inconclusive/insufficient evidence 처리, A/B용 controlled override가 더 정확하다.

**"하드코딩 답변" 질문에 대한 답변 기준**: OpenManager는 정해진 문장을 반환하는 bot이 아니라, deterministic monitoring tool이 계산한 fact와 KB evidence를 LLM이 설명하는 구조다. 다만 portfolio/free-tier 트랙에서는 live ingestion, 장기 저장, RBAC/audit, incident workflow가 제한되므로 Datadog/New Relic/Dynatrace 수준의 enterprise AIOps parity를 주장하면 안 된다. 공개 표현은 "Operational Decision Support AI Assistant" 또는 "Server Monitoring / Observability AI Assistant"가 가장 정확하다.
...
future target에서는 이 contract가 Cloud Run Planner가 생성한 authoritative plan으로 승격되고, `AssistantResult`가 typed `ArtifactEnvelope` 또는 chat result payload를 직접 운반한다. 그 단계에서 `EvidenceCard`는 현재 `cloud-run/ai-engine/src/lib/retrieval-contract.ts`의 계약을 우선 재사용하고, artifact/error union은 실제 코드의 artifact/error 타입과 맞춰 확정한다.
...
반대로 `AssistantPlan`/`AssistantResult`, `routeDecision`, `ArtifactEnvelope`, provider smoke freshness는 대체 설계가 아니라 **Option A 개선 항목**이다. 이 항목들은 §7의 적용 시나리오에서 다룬다.
...
### Option A. ✅ 채택 — Next.js BFF + Cloud Run deterministic/single-first AI Runtime

```text
Browser -> Next.js BFF -> Cloud Run Supervisor -> deterministic/single path
                                             \-> conditional agent escalation
                         \-> Redis/Cloud Tasks for long jobs
```

| 항목 | 평가 |
|------|------|
| 장점 | 긴 실행/비동기 작업/조건부 multi-agent escalation/쿼터 방어를 수용하기 쉽다. 무료 tier 운영 제약과 맞다. |
...
**채택 이유**: A는 job queue가 없으면 처리 못 하는 장시간 분석(B의 한계)과, artifact별 운영 복잡도(D의 단점) 사이에서 현실적 균형점이다. 다만 A의 운영 철학은 과거의 "multi-agent first"가 아니라 현재의 **deterministic/single-first + conditional multi-agent escalation**로 재정의한다. C와 E의 원칙은 A 구조 안에서 점진적으로 흡수한다.
...
**상용 제품 전제에서는 별도 평가**: paid LLM API와 안정적인 quota/SLA, live telemetry ingestion, 장기 저장, RBAC/audit, incident workflow가 가능하다면 이 표의 점수는 그대로 재사용할 수 없다. 그 경우에는 multi-provider 무료 fallback과 BFF/job queue 복잡도는 점수에서 감점될 수 있고, Enterprise AIOps형 통합 backend, managed agent platform, durable workflow, artifact workspace/schema registry가 더 높은 점수를 받을 가능성이 있다.
...
| Mode decision | deterministic/single-first + conditional multi-agent escalation | 비용과 품질 균형을 잡으려는 설계다. 단순 metric lookup/server snapshot은 multi-agent로 올리지 않는다. |
...
| 보강 | 이유 |
|------|------|
| authoritative `AssistantPlan` 승격 | M6 wrapper-only facade 실험과 M7 fact boundary는 완료됐고, `/api/ai/ask` facade는 2026-05-07 제거됐다. 다음 단계는 route catalog 정리, drift/latency 기준 통과 후 routing authority 점진 이전이다. |
...
2026-05-07 현재 코드 기준으로 실제 authority와 contract 위치를 다시 확인했다. 결론은 문서의 큰 방향과 일치한다. **M1~M7은 관측·정렬·historical wrapper 실험·fact boundary 계층이고, 실제 routing authority 이전은 아직 아니다.**

| 표면 | 현재 코드 근거 | 평가 |
|------|---------------|------|
| Frontend route authority | `src/hooks/ai/core/useQueryExecution.ts`가 `buildFrontendQueryRoutingDecision()` 결과로 streaming/job queue를 직접 고른다. `src/hooks/ai/core/query-routing.test.ts`는 thinking mode에서 `autoJobCount 2/6 -> thinkingJobCount 4/6`을 고정한다. | thinking 버튼은 실질 차이가 있다. planner authority는 아직 frontend에 남아 있으므로 authority 이전은 별도 gate가 필요하다. |
...
| BFF stream proxy | `src/app/api/ai/supervisor/stream/v2/route.ts`가 `analysisMode`, `queryAsOf`, sanitized `localRouteDecision`을 Cloud Run으로 전달한다. | 기본 채팅 entrypoint는 이 stream route로 수렴했다. 남은 일은 route catalog 관리와 authority 이전 기준 축적이다. |
...
1. M6 `/api/ai/ask`는 기존 stream/job/artifact route를 **wrapper-only**로 감싸는 방식으로 실험 완료 후 2026-05-07 제거됐다. 새 planner를 새 facade에서 다시 만들면 A의 복잡도가 줄지 않는다.
...
8. 기존 route surface 위에 `/api/ai/ask` facade를 wrapper-only로 추가해 contract 수렴 가능성을 검증했다. **완료: 2026-05-03 M6 / 제거: 2026-05-07 dead-code cleanup**
...
M1~M7과 Streaming UI S1~S3는 기존 라우팅을 한 번에 바꾸지 않고 공통 metadata, shadow 측정, historical wrapper 실험, deterministic fact boundary, stream progress visibility를 만든 단계다. 남은 gap은 새 runtime 도입이 아니라 route catalog 정리, artifact workspace/schema registry, provider reasoning capability policy 정밀화, factPack consumer 확대다.
...
| M6 route-surface experiment | 완료 후 제거. `/api/ai/ask`가 stream/job/incident-report/monitoring-analysis 기존 route를 내부 위임으로 감싸는 방식을 검증했고, 2026-05-07 dead-code cleanup에서 삭제됐다. | route authority는 아직 이전하지 않았다. facade 제거 후에도 기존 stream/job/artifact route catalog 관리는 계속 필요하다. | 새 entrypoint를 늘리기보다 기존 route contract와 `AssistantPlan`/`AssistantResult` metadata를 정렬 |
...
| Streaming UI S1~S3 | 완료 및 v8.11.88 배포. 전체 페이지 SSE, cold start 카운트다운, Cloud Run `agent-step` event와 frontend handling이 들어갔다. | stream event contract와 QA evidence를 계속 유지해야 한다. | 기본 stream route가 progress, cold start, tool step, partial result를 일관된 의미로 표시 |
...
| 우선순위 | 질문 | 판단 기준 |
|:--:|------|-----------|
| 1 | AI route surface를 어디까지 줄이고 어떤 route catalog를 canonical로 둘 것인가? | `/api/ai/ask` facade는 제거됐고 기본 stream/job/artifact route가 남았다. authority를 즉시 이전하면 route surface가 줄지 않고 다시 늘어날 수 있다. |
...
- Grafana Sift investigations: https://grafana.com/docs/grafana-cloud/machine-learning/sift/sift/
...
- [Frontend vs Backend AI Assistant 비교 분석](../reference/architecture/ai/frontend-backend-comparison.md)
...
- [System Architecture Current](../reference/architecture/system/system-architecture-current.md)
...
| 2026-05-03 | `routeDecision` read-only metadata M1 구현 | 라우팅 동작은 유지하되 frontend stream/job/artifact, BFF job, Cloud Run stream done/job result metadata가 같은 `RouteDecision` shape를 보존하도록 정렬. `AssistantPlan`/`AssistantResult` 공통 metadata와 routing authority 이전은 다음 단계로 유지 |
| 2026-05-03 | `AssistantPlan`/`AssistantResult` read-only metadata M2 구현 | 기존 라우팅 동작은 유지하고 `RouteDecision`을 plan/result metadata로 감싸 frontend artifact/stream/job metadata, BFF job response/Redis metadata, Cloud Run supervisor stream done/job result metadata, history/restore/SSE 경로에 보존. 당시 `/api/ai/ask`와 routing authority 이전은 후속 과제로 유지했고, `/api/ai/ask`는 2026-05-07 제거됨 |
...
| 2026-05-05 | 현재 방식 vs 대안 방식 결론 표 추가 | 별도 신규 문서를 만들지 않고 기존 비교 문서 초반에 일반 LLM 챗봇, multi-agent first, rule-only, heavy RAG/GraphRAG, managed agent platform, Enterprise AIOps, OpenManager 현재 방식을 비교하는 요약 표를 통합. Option A 명칭을 현재 구현에 맞게 deterministic/single-first AI Runtime으로 보정 |
| 2026-05-05 | 포트폴리오 트랙과 상용 제품 트랙 평가 분리 | 현재 Option A 채택 판단은 무료 티어/분산 플랫폼/포트폴리오 제약 기준임을 명시. paid LLM API, 단일 제품 backend, live telemetry, 장기 저장, RBAC/audit/incident workflow를 전제로 한 상용 제품 평가는 별도 점수화 대상으로 분리 |
| 2026-05-05 | 범용 포지셔닝 명확화 | 분류를 `운영 의사결정 AI 어시스턴트`로 정리하고, 서버 모니터링은 적용 도메인으로 분리. 완전 자율 AIOps/SRE가 아니라 deterministic fact boundary + LLM 설명 + 운영자 실행을 전제로 한 advisory assistant임을 결론 표에 반영 |
| 2026-05-05 | Observability AI 벤치마크 확장 | Datadog Bits AI SRE, New Relic AI, Elastic Observability AI Assistant, Dynatrace Intelligence RCA, Grafana Sift를 공식 문서 기준으로 비교. 업계 공통 패턴을 telemetry/tool/query engine + evidence/scope guard + LLM narration으로 정리하고, RAG는 조직 운영 지식용 layer로 한정 |
