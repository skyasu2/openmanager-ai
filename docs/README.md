...
OpenManager의 실제 작업 흐름은 아래 한 경로로 추적합니다. 세부 절차는 각 단계의 SSOT 문서에 남기고, 이 문서는 진입점만 제공합니다.
...
작업 파악
  ...
  -> 로컬 deterministic 검증
  -> GitLab CI validate
  -> semver tag production deploy
  -> smoke + 실환경 QA
  -> reports/qa evidence 기록
```

| 단계 | 기준 문서/명령 | 확인할 것 |
|---|---|---|
| 작업 파악 | [TODO](../reports/planning/TODO.md), [AI Standards](./guides/ai/ai-standards.md) | active task, Free Tier, 배포 권위, OTel SSOT |
| 개발환경 | [Development](./development/README.md), [Project Setup](./development/project-setup.md) | WSL2, Node/npm, env, AI CLI/MCP |
| 구현 기준 | [Design](./design/README.md), [Reference](./reference/README.md) | 모듈 책임, API/AI/data 계약 |
| 로컬 검증 | [Test Strategy](./guides/testing/test-strategy.md), `npm run type-check`, `npm run lint`, `npm run test:quick`, `npm run test:contract` | deterministic local gate 통과 |
| 배포 | [Deployment Guide](./operations/deployment-guide.md), [CI/CD](./development/ci-cd.md) | GitLab canonical, semver tag pipeline, Vercel/Cloud Run smoke |
| QA evidence | [QA Reports](../reports/qa/README.md), [QA Status](../reports/qa/QA_STATUS.md) | run JSON, durable artifact, open gap, usage check |
...
| 개발부터 QA까지 어떤 흐름으로 진행했는가 | 이 문서의 [개발/검증/배포/QA 흐름](#개발검증배포qa-흐름), [Development](./development/README.md), [QA Reports](../reports/qa/README.md) |
