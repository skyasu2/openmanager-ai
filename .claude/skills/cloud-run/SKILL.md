---
name: cloud-run
description: Deploy ai-engine to Cloud Run, check GCP cost risk, and verify CLI access. GitLab CI deploy_ai_engine is the production deploy authority; direct deploy.sh is fallback only. Use when user asks to deploy, check billing, verify cloud access, or review cost status.
version: v2.1.0
user-invocable: true
...
- 런타임 SSOT 존재 확인: `test -f public/data/otel-data/resource-catalog.json`
- Docker 준비 확인: `docker ps` (WSL에서 socket 불가 시 fallback: `cmd.exe /c docker ps`)

1. 로컬 빌드 선검증.
- `cd cloud-run/ai-engine && npm run docker:preflight`
- 빌드 전용 모드: `cd cloud-run/ai-engine && SKIP_RUN=true npm run docker:preflight`
- 실패 시 배포 중단 후 원인 해결
...
- CI 경로: `git-workflow`로 GitLab push 후 `npm run gitlab:pipeline:head -- --wait`로 pushed `HEAD` pipeline 확인
  - pipeline이 nonterminal 상태면 `npm run gitlab:pipeline:inspect -- --pipeline <id>`로 세부 확인
...
- 2026-06-06: v2.1.0 - resource-catalog.json preflight 체크 추가, WSL Docker socket fallback 추가, nonterminal pipeline inspect 단계 추가 (Codex adapter 동기화)
