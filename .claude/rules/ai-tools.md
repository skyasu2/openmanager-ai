...
> **스킬 경로**: Claude source = `.claude/skills/` | Codex/Gemini workspace canonical = `.agents/skills/` | Gemini legacy path = `.gemini/skills/` (중복 discovery 방지를 위해 기본 미사용) | Codex user mirror = `npm run skills:sync:codex` → `~/.codex/skills/`
