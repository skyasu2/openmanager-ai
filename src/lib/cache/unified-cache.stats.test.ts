...
describe('buildCacheStats', () => {
  it('returns correct hit rate', () => {
    const stats: UnifiedCacheStatsState = { hits: 3, misses: 4, sets: 5, deletes: 1 };
  ...
  it('returns 0 hit rate when no requests', () => {
    const stats: UnifiedCacheStatsState = { hits: 0, misses: 0, sets: 0, deletes: 0 };
    const cache = new Map<string, CacheItem<unknown>>();
  ...
  it('passes through hits, misses, sets, and deletes', () => {
    const stats: UnifiedCacheStatsState = { hits: 10, misses: 5, sets: 20, deletes: 3 };
    const cache = new Map<string, CacheItem<unknown>>();
    const result = buildCacheStats(stats, cache, 100);
