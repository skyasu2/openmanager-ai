...
describe('getOTelDashboardData', () => {
  ...
  describe('success', () => {
    ...
    it('sorts servers by status priority (critical/offline first, then warning, then online)', async () => {
      ...
      expect(ids.indexOf('srv-critical')).toBeLessThan(
        ids.indexOf('srv-warning')
      );
      expect(ids.indexOf('srv-offline')).toBeLessThan(
        ids.indexOf('srv-warning')
      );
      expect(ids.indexOf('srv-warning')).toBeLessThan(
        ids.indexOf('srv-online')
      );
    });
    ...
    it('converts lastUpdate to Date object', async () => {
      ...
      expect((result.servers[0].lastUpdate as Date).toISOString()).toBe(
  ...
  describe('edge cases', () => {
    ...
    it('handles multiple servers with mixed statuses including maintenance and unknown', async () => {
      ...
      const criticalOfflineStatuses = statuses.filter(
        (s) => s === 'critical' || s === 'offline'
      );
