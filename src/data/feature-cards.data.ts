...
export const FEATURE_CARDS_DATA: FeatureCard[] = [
  {
    ...
    detailedContent: {
      overview: `운영자가 메트릭 그래프를 직접 해석하지 않아도, 질문 하나로 현재 상태, 원인 분석, 다음 조치안을 받을 수 있도록 설계한 운영 의사결정 AI 어시스턴트입니다. 핵심 수치와 판정은 deterministic fact layer가 책임지고, LLM은 tool-calling과 설명·보고서·조치안 생성을 맡습니다. 내부 지식은 Supabase Postgres Full Text Search를 감싼 search_knowledge_text RPC로 검색하고, 원본 지식은 repo 문서와 seed JSON에 남겨 재생성 가능한 인덱스로 관리합니다. 텍스트 에이전트는 Groq·Mistral·Z.AI·Cerebras 4개 Provider를 Round-Robin으로 순환 사용하며, Vision은 Gemini Flash-Lite 전용 경로를 씁니다. RCA/report/advisor/vision처럼 복잡한 요청만 5개 라우팅 에이전트로 escalation합니다. 경량 커스텀 TypeScript ML, Knowledge Retrieval Lite(Postgres FTS + metadata boost), 요청 기반 웹 검색을 분리해 무료 티어 사용량을 예측 가능하게 유지합니다.`,
      features: [
        '⚡ Groq Cloud: LPU 기반 초고속 추론 (meta-llama/llama-4-scout-17b-16e-instruct, 131K ctx) — Round-Robin 텍스트 에이전트 Pool 1번 슬롯',
        '🌊 Mistral AI: mistral-small-latest (32K ctx) — Round-Robin Pool 2번 슬롯, 무료 티어 친화적 텍스트 처리',
        '✨ Z.AI: GLM Flash (128K ctx) — Round-Robin Pool 3번 슬롯, 긴 컨텍스트 텍스트 처리',
        '🧠 Cerebras Inference: gpt-oss-120b (65K ctx, primary) — Round-Robin Pool 4번 슬롯, 초고속 추론 인프라 (llama3.1-8b 2026-05-27 deprecated)',
      ...
      technologies: [
        'Groq Cloud (Round-Robin #1, 131K)',
        'Mistral AI (Round-Robin #2, 32K)',
        'Z.AI GLM Flash (Round-Robin #3, 128K)',
        'Cerebras Inference gpt-oss-120b (Round-Robin #4, 65K)',
