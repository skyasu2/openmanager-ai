...
export const FEATURE_CARDS_DATA: FeatureCard[] = [
  {
    ...
    detailedContent: {
      overview: `운영자가 메트릭 그래프를 직접 해석하지 않아도, 질문 하나로 현재 상태, 원인 분석, 다음 조치안을 받을 수 있도록 설계한 AI 실행 계층입니다. 4개의 AI Provider(Cerebras, Groq, Mistral, Gemini)와 Vercel AI SDK 6.0 기반의 고도화된 5-Agent 멀티에이전트 오케스트레이션을 사용하며, Vision Agent의 대시보드 스크린샷 분석, 1M 토큰 로그 분석, Google Search Grounding을 지원합니다. 경량 커스텀 TypeScript ML(통계 이상 탐지 + 추세 예측)과 Native GraphRAG + Tavily 하이브리드 검색을 함께 사용합니다.`,
      features: [
        ...
        '🛡️ Mistral AI: mistral-large-latest Frontier 모델 — Advisor 1순위, 복잡한 추론 및 RAG 품질 최적화 담당',
        ...
        '🦙 Native GraphRAG: 외부 GraphRAG 프레임워크에 의존하지 않고, Vercel AI SDK + Supabase 조합으로 관계 추출과 검색 흐름을 직접 구성',
