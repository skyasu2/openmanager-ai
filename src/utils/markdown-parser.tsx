...
export function normalizeAssistantMarkdown(content: string): string {
  ...
  return (
    content
      ...
      .replace(/([^\n|`-])[ \t]+(-{3,})[ \t]+(?=\S)/g, '$1\n\n$2\n\n')
      // 2) 줄 시작 수평선에 본문이 붙은 경우: "--- 결론:" → hr + 본문 분리
      //    (대시만 단독인 정상 hr 와 GFM 테이블 구분자(|---|)는 영향 없음)
      .replace(/(^|\n)(-{3,})[ \t]+(?=\S)/g, '$1$2\n\n')
      // 2b) 줄 끝에 붙은 수평선: "텍스트 ---<줄끝>" → hr 분리
      //     (리스트 불릿 "- ---" 과 테이블 구분자 "| --- |" 는 직전 문자가 제외되어 무손상)
      .replace(/([^\s|`-])[ \t]+(-{3,})[ \t]*$/gm, '$1\n\n$2\n')
      // 3) 공백 없이 붙은 compact 헤딩 앞 줄바꿈: "텍스트##제목"
      .replace(/([^#\n\s])(#{2,6})(?=[^\s#])/g, '$1\n\n$2')
      // 4) 줄 중간에 공백을 둔 헤딩 앞 줄바꿈: "텍스트 ## 제목"
      .replace(/([^\n])[ \t]+(#{2,6}[ \t])/g, '$1\n\n$2')
      // 5) 라인 시작 compact 헤딩 공백 보정: "##제목" → "## 제목"
      .replace(/(^|\n)(#{1,6})([^\s#])/g, '$1$2 $3')
      // 6) 빈 bold 마커 제거: 정확히 4개 연속 별표("****")는 빈 강조 아티팩트
      .replace(/(?<!\*)\*{4}(?!\*)/g, '')
      // 7) bold 구분자 안쪽 가장자리 공백 정리: "** 텍스트 **" → "**텍스트**"
      //    닫는 "**" 앞 공백은 CommonMark 에서 강조를 깨므로 가장자리 공백만 제거.
      //    내부 공백("**a b**")과 정상 bold("**a**")는 보존된다.
      .replace(/\*\*[ \t]*([^*\n]+?)[ \t]*\*\*/g, '**$1**')
  );
}
