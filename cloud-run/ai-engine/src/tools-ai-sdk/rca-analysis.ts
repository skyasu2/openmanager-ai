...
function getCurrentRcaCacheSlotIndex(): number {
  return calculateRelativeDateTime(0).slotIndex;
}
...
export const buildIncidentTimeline = tool({
  ...
  execute: async ({
    ...
    try {
      ...
      const slotIndex = getCurrentRcaCacheSlotIndex();

      return await cache.getAnalysis(
        'rca-timeline',
        { serverId, timeRangeHours, slotIndex },
        async () => {
...
export const correlateMetrics = tool({
  ...
  execute: async ({
    ...
    try {
      ...
      return await cache.getAnalysis(
        'rca-correlation',
        { serverId, targetMetric, slotIndex },
        async () => {
