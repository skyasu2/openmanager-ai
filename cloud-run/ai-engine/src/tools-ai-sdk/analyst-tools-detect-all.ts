...
import {
  ...
  type LightweightEvidenceContract,
...
export type AllServerMetricType = 'cpu' | 'memory' | 'disk' | 'network' | 'all';
export type ExternalServerInput = z.infer<typeof ExternalServerSchema>;

export interface AllServerAnomalyScanInput {
  metricType: AllServerMetricType;
  externalServers?: ExternalServerInput[];
}

export interface AllServerAnomalyScanResult {
  success: true;
  totalServers: number;
  ...
  evidenceContract: LightweightEvidenceContract;
...
export async function runAllServerAnomalyScan({
  metricType,
  externalServers,
}: AllServerAnomalyScanInput): Promise<AllServerAnomalyScanResult> {
  ...
  return await cache.getAnalysis(
    ...
    async () => {
      ...
      const externalHistory: ExternalMetricHistory | undefined = externalServers
        ? Object.fromEntries(
            externalServers
              .filter((server) => server.history)
              .map((server) => [
                server.id,
                {
                  ...(server.history?.cpu ? { cpu: server.history.cpu } : {}),
                  ...(server.history?.memory
                    ? { memory: server.history.memory }
                    : {}),
                  ...(server.history?.disk ? { disk: server.history.disk } : {}),
                  ...(server.history?.network
                    ? { network: server.history.network }
                    : {}),
                },
              ])
          )
      ...
      const targetMetrics = metricType === 'all' ? metrics : [metricType];
      ...
      for (const server of allServers) {
        ...
        for (const metric of targetMetrics) {
          const currentValue = Number(server[metric as keyof typeof server] ?? 0);
          ...
          const recentHistory = history
            .slice(-TREND_WINDOW_SLOTS)
            .map((point) => point.value);
          const projectedValue30m =
            Math.round(projectValue(recentHistory, TREND_HORIZON_SLOTS) * 10) /
            10;
          const isFutureWarning =
            currentValue < threshold.warning &&
            projectedValue30m >= threshold.warning;
          ...
          if (isFutureWarning) {
            risingTrends.push({
              ...
              riskLevel:
                projectedValue30m >= threshold.critical ? 'high' : 'medium',
            });
          }
...
export const detectAnomaliesAllServers = tool({
  ...
  execute: async ({
    metricType,
    externalServers,
  }: AllServerAnomalyScanInput) => {
    try {
      return await runAllServerAnomalyScan({ metricType, externalServers });
    } catch (error) {
      return {
