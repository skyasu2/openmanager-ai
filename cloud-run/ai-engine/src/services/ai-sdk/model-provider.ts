 ...
 * Fallback chain: Groq → Z.AI → Mistral → Cerebras with CB check
 *
...
export function getVisionAgentModel(): {
  model: LanguageModel;
  provider: 'gemini' | 'openrouter' | 'zai';
  modelId: string;
} | null {
...
export async function checkAllProvidersHealth(): Promise<ProviderHealth[]> {
  ...
  if (status.zai) {
    results.push(await testProviderHealth('zai'));
  }
  if (status.gemini) {
...
export async function getSupervisorModelWithQuota(
  ...
  // Provider priority: Groq text primary > Z.AI > Mistral > Cerebras.
  const preferredOrder: QuotaProviderName[] = ['groq', 'zai', 'mistral', 'cerebras'];
  ...
  if (selection) {
    ...
    switch (provider) {
      ...
      case 'zai': {
        const zaiModelId = getZaiModelId();
        return {
          model: getZaiModel(zaiModelId),
          provider: 'zai',
          modelId: zaiModelId,
          isPreemptiveFallback,
        };
