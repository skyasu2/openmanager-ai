...
const INTERNAL_PATH_TARGET_PATTERN =
  /내부\s*(자료|문서|파일|경로|데이터|저장소|상태)|파일\s*경로|자료\s*경로|데이터\s*경로|문서\s*경로|저장\s*경로|repo\s*path|repository\s*path|source\s*path|경로|위치|파일|소스|코드|구현|정의|path|file|source|code|implementation|defined|config|환경\s*변수|env|secret|system\s*prompt|시스템\s*프롬프트/i;
...
export function buildInternalImplementationPathRefusal(): string {
  return [
