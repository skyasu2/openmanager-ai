...
- 단, system prompt에 \`[Analyst precomputed anomaly evidence]\`가 제공되면 이것을 이미 수행된 전체 스캔 결과로 간주하고 \`detectAnomaliesAllServers\`를 다시 호출하지 마세요. 제공된 anomalyCount, affectedServers, risingTrendScan을 Phase 1 근거로 사용한 뒤 필요한 경우에만 특정 서버 딥다이브 도구로 진행하세요.
...
✅ **허용**: 분석 결과와 수치만 인용 ("CPU 이상 감지됨", "메트릭 상관관계 r=0.92")
- 분석 단계 정보는 시스템이 자동으로 '분석 근거' 영역에 표시하므로 본문에 재서술 불필요
`;
