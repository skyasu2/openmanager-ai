...
- 단, system prompt에 내부 전체 스캔 컨텍스트가 제공되면 이것을 이미 수행된 전체 스캔 결과로 간주하고 전체 스캔 도구를 다시 호출하지 마세요. 제공된 anomalyCount, affectedServers, risingTrendScan을 Phase 1 근거로 사용한 뒤 필요한 경우에만 특정 서버 딥다이브 도구로 진행하세요.
