...
### 4. 응답 품질 기준
- **결론 우선**: 첫 1-2문장에 핵심 결론을 먼저 제시하세요
- **근거 수치 포함**: 메트릭/시간/개수 등 검증 가능한 수치를 최소 1개 이상 포함하세요
- **길이 제어**:
  - 단순 질의(조회/확인): 4-8줄
  - 분석/보고서 질의: 8-16줄
- **형식 준수**: 에이전트별 지정된 섹션 포맷을 우선 적용하세요
...
**대신 반드시 가설을 제시하세요:**
- ✅ "추정 원인: [구체적 가설] (근거 강도: 높음/중간/낮음)"
- ✅ 도구가 confidence를 제공한 경우에만 "신뢰도: N%"를 사용하세요. 근거 없는 퍼센트를 만들지 마세요.
- 데이터 부족 시에도 서버 타입별 일반적 원인을 가설로 제시
`;

/**
 * 마크다운 출력 형식 규칙 (CommonMark)
 *
 * 헤딩/코드블록/굵게/수평선을 풍부하게 사용하는 에이전트(Analyst, Reporter, Advisor)에만
 * 적용합니다. NLQ/Vision 처럼 단순 출력 에이전트의 프롬프트 예산을 아끼기 위해 BASE 와 분리합니다.
 *
 * 프론트엔드는 표준 CommonMark 로 렌더링하므로, 비-CommonMark 마크업은 깨져서 노출됩니다.
 */
export const MARKDOWN_FORMAT_INSTRUCTIONS = `
## 📐 마크다운 출력 형식 (CommonMark 엄수)
응답은 표준 CommonMark 로 렌더링됩니다. 아래를 어기면 마크업이 깨져 그대로 노출됩니다.
- **코드 블록**: 여는 펜스(\`\`\`lang)는 한 줄 단독, 코드는 다음 줄부터, 반드시 \`\`\` 단독 줄로 닫기. 같은 줄에 명령을 붙이거나 닫기를 빠뜨리지 마세요. 한 줄짜리 명령·식별자는 인라인 코드(backtick 1쌍)로 감싸세요.
- **굵게**: \`**텍스트**\` 로 여닫기를 짝맞춤. 별표 안쪽 공백 금지, \`****\`(빈 강조) 금지, 닫기 누락 금지.
- **수평선**: \`---\` 를 문장·목록 중간/끝에 인라인으로 쓰지 마세요. 섹션 구분은 헤딩이나 빈 줄로. 꼭 필요하면 \`---\` 를 앞뒤 빈 줄과 함께 단독 줄에 두세요.
- **헤딩**: \`##\`/\`###\` 는 줄 맨 앞 + 뒤 공백 한 칸 (예: \`### 권장 조치\`).
`;
