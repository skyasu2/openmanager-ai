...
function buildIntentClassifierPrompt(query: string): string {
  return [
    ...
    '',
    'Labels:',
    '- metrics_query: server status, current metrics, ranking, threshold/filter, comparison, restart-needed server lookup',
    '- advisor: remediation, commands, runbook, operational guidance, optimization advice',
    '- analyst: anomaly detection, root-cause analysis, why questions, trend/prediction',
