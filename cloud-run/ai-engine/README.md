...
모델 실행 경로에는 응답 품질과 장애 복구를 위한 가드레일을 둡니다.

1. [assistant-runtime-host.ts](src/services/ai-sdk/assistant-runtime-host.ts): 도메인 툴과 요청 컨텍스트를 모델 호출에 바인딩합니다.
2. [supervisor-stream-text-guard.ts](src/services/ai-sdk/supervisor-stream-text-guard.ts): 스트림 텍스트와 비정상 도구 호출을 필터링합니다.
3. [supervisor-quality-retry.ts](src/services/ai-sdk/supervisor-quality-retry.ts), [supervisor-stream-recovery.ts](src/services/ai-sdk/supervisor-stream-recovery.ts): 품질 미달 응답과 스트리밍 탈락을 재시도/복구합니다.
...
외부 LLM API 제공업체들(Cerebras, Groq 등)의 무료 티어는 타이트한 속도 제한을 가집니다. 런타임 레이어에서 쿼타를 추적하고 fallback 경로를 선택합니다.
