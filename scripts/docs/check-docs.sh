...
echo -e "\n${YELLOW}[1/8] Markdown Lint 검사${NC}"
...
echo -e "\n${YELLOW}[2/8] 내부 링크 유효성 검사 (docs 전체)${NC}"
...
# 3. API 엔드포인트 카탈로그 정합성 검사
echo -e "\n${YELLOW}[3/8] API 엔드포인트 카탈로그 정합성 검사${NC}"
if npm run --silent docs:api:endpoints:check 2>&1 | tee "$REPORTS_DIR/api-endpoints.log"; then
  echo -e "${GREEN}✅ API 엔드포인트 카탈로그 정합성 검사 통과${NC}"
else
  echo -e "${RED}❌ API 엔드포인트 카탈로그 정합성 검사 실패${NC}"
  HAS_ERROR=1
fi

# 4. AI 운영 문서 정합성 검사
echo -e "\n${YELLOW}[4/8] AI 운영 문서 정합성 검사${NC}"
