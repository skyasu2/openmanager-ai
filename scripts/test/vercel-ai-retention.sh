...
is_launch_failure() {
  grep -Eq 'browserType\.launch|setsockopt: Operation not permitted|signal=SIGTRAP|Target page, context or browser has been closed' "$LOG_FILE"
}
...
if is_launch_failure; then
  FALLBACK_CHANNEL="$(alternate_channel "$CHANNEL")"
  echo "Launch failure detected. Retrying on fallback channel: $FALLBACK_CHANNEL"
  sleep 2
